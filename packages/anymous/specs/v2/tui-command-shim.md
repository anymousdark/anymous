...
Delete `packages/anymous/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/anymous/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/anymous/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/anymous`
- TUI plugin loader tests in `packages/anymous` if runtime plugin API wiring changed
