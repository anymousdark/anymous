...
Practical reference for a future `packages/server` split after the anymous
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/anymous`.
...
Until enough shared service code lives outside `packages/anymous`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/anymous`

It should not import `packages/anymous` services while `packages/anymous`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on anymous-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/anymous` implementation details.
