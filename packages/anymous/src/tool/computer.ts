  ...
  Effect.promise<string>(
    () =>
      new Promise((resolve, reject) => {
        ...
        const proc = spawn("powershell", ["-NoProfile", "-NonInteractive", "-EncodedCommand", enc])
        let stdout = ""
        let stderr = ""
...
`)
    ...
    const action =
      button === "left"
        ? "click"
        : button === "right"
          ? "click at (get position of mouse) using {button 2}"
          : "click at (get position of mouse) using {button 3}"
    yield* Effect.promise(
      () =>
        new Promise<void>((resolve, reject) => {
          const proc = spawn("osascript", ["-e", `tell application "System Events" to ${action}`])
          proc.on("close", (code) => (code === 0 ? resolve() : reject(new Error(`exit ${code}`))))
          proc.on("error", reject)
        }),
    )
      ...
      return Effect.promise<void>(
        () =>
          new Promise((resolve, reject) => {
            const proc = spawn("osascript", [
              "-e",
              `tell application "System Events" to keystroke "${char.replace(/"/g, '\\"')}"`,
            ])
            proc.on("close", (code) => (code === 0 ? resolve() : reject(new Error(`exit ${code}`))))
            proc.on("error", reject)
          }),
...
`)
    ...
    yield* Effect.promise(
      () =>
        new Promise<void>((resolve, reject) => {
          const proc = spawn("osascript", ["-e", `tell application "System Events" to scroll wheel ${clicks} lines`])
          proc.on("close", (code) => (code === 0 ? resolve() : reject(new Error(`exit ${code}`))))
          proc.on("error", reject)
        }),
    )
...
`)
  ...
  if (platform === "darwin") {
    const raw = yield* Effect.promise<string>(
      () =>
        new Promise((resolve, reject) => {
          const proc = spawn("osascript", ["-e", `tell application "Finder" to get bounds of window of desktop`])
          let d = ""
...
export const Action = Schema.Struct({
  ...
  delayMs: Schema.optional(Schema.Number).annotate({
    description: "Delay in ms between keystrokes (for type action, default: 0)",
  }),
  keys: Schema.optional(Schema.Array(Schema.String)).annotate({
    description: "Keys to press (for keypress action), e.g. ['ctrl', 'c']",
  }),
