...
//   2. Interactive local (`anymous --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`anymous --mini --attach`): connects to a running
//      anymous server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run Anymous with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running Anymous server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ANYMOUS_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to ANYMOUS_SERVER_USERNAME or 'anymous')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: anymousClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: anymousClient, sessionID: string) {
      ...
      async function current(sdk: anymousClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: anymousClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: anymousClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: anymousClient) {
        ...
        async function loop(client: anymousClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createanymousClient({
        baseUrl: "http://anymous.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
