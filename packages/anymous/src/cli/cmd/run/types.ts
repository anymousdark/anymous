// Shared type vocabulary for the direct interactive mode (`anymous --mini`).
//
...
import type { anymousClient, PermissionRequest, QuestionRequest, ToolPart } from "@anymous-ai/sdk/v2"
import type { TuiConfig } from "@anymous-ai/tui/config"
...
type PromptModel = Parameters<anymousClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<anymousClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<anymousClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<anymousClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<anymousClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<anymousClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<anymousClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<anymousClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<anymousClient["question"]["reject"]>[0]
