...
//
// Renders the full Anymous entry logo and a compact [A] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "any⋅mous", right, undefined, TextAttributes.BOLD)
    push(lines, body_left + "any⋅mous".length + 2, top, "v" + VERSION, left, undefined, TextAttributes.DIM)

    const subtitle = "AI-Powered Reverse Engineering & Pentest Platform"
    push(lines, body_left, top + 1, Locale.truncateMiddle(subtitle, Math.max(1, width - body_left)), left, undefined)
    ...
    height = top + mark.length + (input.detail ? 1 : 0)
  }
  ...
  if (kind === "exit") {
    ...
    push(lines, body_left, top, "any⋅mous", left, undefined, TextAttributes.DIM)
    if (input.showSession !== false) {
      push(lines, body_left + "any⋅mous".length + 2, top, meta.title, right, undefined, TextAttributes.BOLD)
    }
    ...
    push(
      ...
      top + 1,
      `anymous --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
