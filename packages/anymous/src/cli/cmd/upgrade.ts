...
export const UpgradeCommand = {
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("sync", {
        alias: "s",
        describe: "sync with latest anymous release before upgrading",
        type: "boolean",
        default: false,
      })
  },
  handler: async (args: { target?: string; method?: string; sync?: boolean }) => {
    ...
    // Check for anymous updates
    const anymousLatest = await checkanymousUpdate()
    if (anymousLatest && anymousLatest !== InstallationVersion) {
      prompts.log.info(`anymous v${anymousLatest} disponível (atual: v${InstallationVersion})`)
      if (args.sync) {
        prompts.log.info("Sincronizando com anymous...")
        prompts.log.info(`Execute: bun run script/sync-anymous.ts --version=${anymousLatest}`)
      } else {
        const sync = await prompts.confirm({
          message: `Sincronizar com anymous v${anymousLatest}?`,
          initialValue: false,
        })
        if (sync) {
          prompts.log.info(`Execute para sincronizar:`)
          prompts.log.info(`  bun run script/sync-anymous.ts --version=${anymousLatest}`)
          prompts.log.info(`Depois: cd packages/anymous/dist-npm && npm publish --access public`)
        }
      }
    }
