...
import { Effect, FiberSet, Queue } from "effect"
...
import type { LLMClientShape } from "@anymous-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("anymous"))
    return { type: "unsupported", reason: "provider is not openai, anymous, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @anymous-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains anymous-owned. The native runtime only adapts
      // the @anymous-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
