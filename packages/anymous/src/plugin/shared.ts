...
export const DEPRECATED_PLUGIN_PACKAGES = ["anymous-openai-codex-auth", "anymous-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, anymousVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(anymousVersion) || semver.major(anymousVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.anymous
  if (typeof range !== "string") return
  if (!semver.satisfies(anymousVersion, range)) {
    throw new Error(`Plugin requires anymous ${range} but running ${anymousVersion}`)
  }
}
