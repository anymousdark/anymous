...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@anymous-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@anymous-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@anymous-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@anymous-ai/tui/context/theme"
import { Global } from "@anymous-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@anymous-ai/core/util/flock"
import { Flag } from "@anymous-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@anymous-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@anymous-ai/core/v1/config/plugin"
import { createCommandShim } from "@anymous-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@anymous-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".anymous"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".anymous", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".anymous", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ANYMOUS_PURE ? [] : pluginOrigins
    if (Flag.ANYMOUS_PURE && pluginOrigins.length) {
    }
