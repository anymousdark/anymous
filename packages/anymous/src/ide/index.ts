...
import { IdeEvent } from "@anymous-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["ANYMOUS_CALLER"] === "vscode" || process.env["ANYMOUS_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.anymous"], {
    nothrow: true,
  })
