...
import { isRecord } from "@anymous-ai/tui/util/record"
...
import { TuiConfig } from "@anymous-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@anymous/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old anymous.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.anymous`
  // folders between cwd and home, and ANYMOUS_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ANYMOUS_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ANYMOUS_TUI_CONFIG override, if set.
  if (Flag.ANYMOUS_TUI_CONFIG) {
    const configFile = Flag.ANYMOUS_TUI_CONFIG
  ...
  // 4. `.anymous` directories (and ANYMOUS_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".anymous") || dir === Flag.ANYMOUS_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".anymous") && dir !== Flag.ANYMOUS_CONFIG_DIR) continue
