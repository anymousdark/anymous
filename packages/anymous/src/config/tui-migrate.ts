...
const TUI_SCHEMA_URL = "https://anymous-cli.vercel.app/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from anymous.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const anymous = await anymousFiles(input)
  for (const file of anymous) {
...
async function anymousFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "anymous"),
    ...(await Filesystem.findUp(["anymous.json", "anymous.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "anymous"))
  }
  if (Flag.ANYMOUS_CONFIG) files.push(Flag.ANYMOUS_CONFIG)
