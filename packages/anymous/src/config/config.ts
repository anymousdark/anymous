...
import { InstallationLocal, InstallationVersion } from "@anymous-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@anymous-ai/core/v1/config/console-state"
import { FSUtil } from "@anymous-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@anymous-ai/core/util/effect-flock"
...
export class Service extends Context.Service<Service, Interface>()("@anymous/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://anymous-cli.vercel.app/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.ANYMOUS_CONFIG && !Flag.ANYMOUS_CONFIG_DIR && !Flag.ANYMOUS_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://anymous-cli.vercel.app/config.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://anymous-cli.vercel.app/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ANYMOUS_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/anymous`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://anymous-cli.vercel.app/config.json"
        ...
        if (Flag.ANYMOUS_CONFIG) {
          yield* merge(Flag.ANYMOUS_CONFIG, yield* loadFile(Flag.ANYMOUS_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.ANYMOUS_CONFIG })
        }

        if (!Flag.ANYMOUS_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("anymous", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ANYMOUS_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from ANYMOUS_CONFIG_DIR", { path: Flag.ANYMOUS_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".anymous") || dir === Flag.ANYMOUS_CONFIG_DIR) {
            for (const file of ["anymous.json", "anymous.jsonc"]) {
          ...
          // Auto-discovered plugins under `.anymous/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (process.env.ANYMOUS_CONFIG_CONTENT) {
          const source = "ANYMOUS_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ANYMOUS_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from ANYMOUS_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ANYMOUS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ANYMOUS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ANYMOUS_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ANYMOUS_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("ANYMOUS_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.ANYMOUS_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ANYMOUS_DISABLE_PRUNE) {
