...
import { SkillPlugin } from "@anymous-ai/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@anymous-ai/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@anymous-ai/core/v1/config/error"
...
const ANYMOUS_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with anymous. The model's intuition for what an
// anymous.json should look like is often wrong, and anymous hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch anymous's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_ANYMOUS_SKILL_NAME = "customize-anymous"
const CUSTOMIZE_ANYMOUS_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating anymous's own configuration: anymous.json, anymous.jsonc, files under .anymous/, or files under ~/.config/anymous/. Also use when creating or fixing anymous agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring anymous itself."
const CUSTOMIZE_ANYMOUS_SKILL_BODY = SkillPlugin.CustomizeanymousContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, ANYMOUS_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@anymous/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_ANYMOUS_SKILL_NAME] = {
          name: CUSTOMIZE_ANYMOUS_SKILL_NAME,
          description: CUSTOMIZE_ANYMOUS_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_ANYMOUS_SKILL_BODY,
        }
