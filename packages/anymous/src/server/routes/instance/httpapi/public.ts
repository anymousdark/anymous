...
import { anymousHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for anymous serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(anymousHttpApi)
...
export const PublicApi = anymousHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "anymous",
    version: "1.0.0",
    description: "anymous api",
