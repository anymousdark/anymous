...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.ANYMOUS_TEST ?? "")'), {
      env: {
        ANYMOUS_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "ANYMOUS_TEST_SHELL"], {
      shell: true,
      env: {
        ANYMOUS_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("ANYMOUS_TEST_SHELL=ok")
  })
