...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "anymous.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "anymous.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "anymous.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "anymous.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["anymous.json", "anymous.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "anymous.json"),
        path.join(project, "anymous.jsonc"),
      ])
    })
  })
