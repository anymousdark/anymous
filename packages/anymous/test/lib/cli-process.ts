// Subprocess test harness for the anymous CLI. Spawns the real binary against
...
//
// Configuration flows through anymous's built-in test affordances:
//   - ANYMOUS_CONFIG_CONTENT      : provider config inline, no files to find
//   - ANYMOUS_TEST_HOME           : pins os.homedir() → tmpdir
//   - ANYMOUS_DISABLE_PROJECT_CONFIG : skip walking up for anymous.json
//   - ANYMOUS_PURE                : skip external plugin discovery + install
//   - ANYMOUS_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `anymous.run` is fully wired. The shape supports adding more
// builders (`anymous.serve(opts)`, `anymous.acp(opts)`, `anymous.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of anymousCli.
...
const anymousRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(anymousRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ANYMOUS_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    ANYMOUS_CONFIG_CONTENT: configJson,
    ANYMOUS_DISABLE_PROJECT_CONFIG: "1",
    ANYMOUS_PURE: "1",
    ANYMOUS_DISABLE_AUTOUPDATE: "1",
    ANYMOUS_DISABLE_AUTOCOMPACT: "1",
    ANYMOUS_DISABLE_MODELS_FETCH: "1",
    ANYMOUS_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `anymous run`. New flags should
...
// `anymous serve` is a long-lived process — it never exits on its own.
...
// `anymous acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type anymousCli = {
  ...
  // Spawn `anymous serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `anymous acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("anymous.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          ANYMOUS_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("anymous.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("anymous.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "anymous server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `anymous serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("anymous.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const anymous: anymousCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, anymous })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `anymous.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
// every call site. Use this for any test that needs the anymous CLI fixture.
//
...
// resources (e.g. `anymous.serve`) without an extra `Effect.scoped` wrapper —
