    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".anymous/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const anymousModel = {
      ...
      api: {
        id: "anymous-test",
        url: "https://anymous-cli.vercel.app",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, anymousModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.anymous?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.anymous?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              anymous: { itemId: "msg_ANYMOUS_part" },
    ...
    expect(result[0].providerOptions?.anymous?.itemId).toBe("msg_anymous")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.anymous?.itemId).toBe("msg_ANYMOUS_part")
