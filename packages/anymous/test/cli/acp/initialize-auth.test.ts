...
describe("anymous acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ anymous }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ anymous }))
        ...
        expect(initialized.agentInfo?.name).toBe("anymous")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ anymous }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ anymous })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("anymous-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "anymous-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.ANYMOUS_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ anymous }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("anymous-login")
