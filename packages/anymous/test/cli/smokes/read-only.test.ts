...
describe("anymous read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // ANYMOUS_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ anymous }) =>
      Effect.gen(function* () {
        const r = yield* anymous.spawn(["mcp", "list"])
        anymous.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ anymous }) =>
      Effect.gen(function* () {
        const r = yield* anymous.spawn(["providers", "list"])
        anymous.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ anymous }) =>
      Effect.gen(function* () {
        const r = yield* anymous.spawn(["models"])
        anymous.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ anymous }) =>
      Effect.gen(function* () {
        const r = yield* anymous.spawn(["agent", "list"])
        anymous.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh ANYMOUS_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ anymous }) =>
      Effect.gen(function* () {
        const r = yield* anymous.spawn(["session", "list"])
        anymous.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ anymous }) =>
      Effect.gen(function* () {
        const r = yield* anymous.spawn(["stats"])
        anymous.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under ANYMOUS_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ anymous }) =>
      Effect.gen(function* () {
        const r = yield* anymous.spawn(["db", "path"])
        anymous.expectExit(r, 0, "db path")
