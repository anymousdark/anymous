...
import { anymousClient, type GlobalEvent } from "@anymous-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<anymousClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<anymousClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<anymousClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<anymousClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<anymousClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://anymous.test"),
...
function sdk(
  input: {
    ...
    subscribe?: anymousClient["event"]["subscribe"]
    globalEvent?: anymousClient["global"]["event"]
    promptAsync?: anymousClient["session"]["promptAsync"]
    status?: anymousClient["session"]["status"]
    messages?: anymousClient["session"]["messages"]
    children?: anymousClient["session"]["children"]
    permissions?: anymousClient["permission"]["list"]
    questions?: anymousClient["question"]["list"]
  } = {},
) {
  const client = new anymousClient()

  const subscribe: anymousClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: anymousClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: anymousClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: anymousClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: anymousClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: anymousClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: anymousClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: anymousClient["question"]["list"] = input.questions ?? (() => ok([]))
