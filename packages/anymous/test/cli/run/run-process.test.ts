// Subprocess integration tests for `anymous run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `anymous.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `ANYMOUS_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("anymous run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const result = yield* anymous.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const result = yield* anymous.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* anymous.run("think", { extraArgs: ["--thinking"] })
        anymous.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* anymous.run("think again")
        anymous.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const result = yield* anymous.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const result = yield* anymous.run("say hi", { format: "json" })
        anymous.expectExit(result, 0)

        const events = anymous.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ anymous }) =>
      Effect.gen(function* () {
        const result = yield* anymous.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const result = yield* anymous.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const result = yield* anymous.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* anymous.run("request permission", { permission: { bash: "ask" } })
        anymous.expectExit(denied, 0)
        ...
        const allowed = yield* anymous.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        anymous.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* anymous.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        anymous.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, anymous }) =>
      Effect.gen(function* () {
        ...
        const server = yield* anymous.serve()

        const result = yield* anymous.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, anymous }) =>
      Effect.gen(function* () {
        const result = yield* anymous.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, anymous }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* anymous.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
