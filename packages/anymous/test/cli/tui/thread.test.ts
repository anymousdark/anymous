...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ anymous }) =>
    Effect.gen(function* () {
      const result = yield* anymous.spawn(["--replay-limit", "10"])

      anymous.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ anymous }) =>
    Effect.gen(function* () {
      const result = yield* anymous.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ anymous }) =>
    Effect.gen(function* () {
      const result = yield* anymous.spawn(["--mini", "--port", "4096"])
