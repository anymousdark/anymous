...
`,
  ...
  const pure = process.env.ANYMOUS_PURE
  const meta = process.env.ANYMOUS_PLUGIN_META_FILE
  process.env.ANYMOUS_PURE = "1"
  process.env.ANYMOUS_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    if (pure === undefined) {
      ...
      process.env.ANYMOUS_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.ANYMOUS_PLUGIN_META_FILE
    } else {
      process.env.ANYMOUS_PLUGIN_META_FILE = meta
    }
  }
})
