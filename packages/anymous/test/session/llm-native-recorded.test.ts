...
import { LLMEvent, LLMResponse } from "@anymous-ai/llm"
import { RequestExecutor } from "@anymous-ai/llm/route"
...
const zenURL = (connection: string) => `https://anymous-cli.vercel.app/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.ANYMOUS_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["anymous", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ANYMOUS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("ANYMOUS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["anymous", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "anymous-proxy",
    name: "anymous proxy",
    providerID: ProviderV2.ID.anymous,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["anymous", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.ANYMOUS_RECORD_CONSOLE_TOKEN && process.env.ANYMOUS_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anymous,
        name: "anymous Zen",
        env: ["ANYMOUS_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.ANYMOUS_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.ANYMOUS_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.ANYMOUS_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["anymous", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ANYMOUS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("ANYMOUS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("ANYMOUS_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set ANYMOUS_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the anymous LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "anymous.json"),
      JSON.stringify({ $schema: "https://anymous-cli.vercel.app/config.json", ...scenario.config(model) }),
    ),
  )
