...
it.instance("plan agent denies edits except .anymous/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".anymous/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".anymous", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.ANYMOUS_TEST_HOME
      process.env.ANYMOUS_TEST_HOME = test.directory
