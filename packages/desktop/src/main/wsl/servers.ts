...
import { expectanymousVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveanymous?: typeof resolveWslanymous
...
export function createWslServersController(
  ...
  const setanymousCheck = (distro: string, check: WslanymousCheck) => {
    setState({
      anymousChecks: {
        ...state.anymousChecks,
        [distro]: check,
  ...
  const checkanymous = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveanymous ?? resolveWslanymous)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return anymousCheck(distro, resolved, version, appVersion)
  }

  const refreshanymousCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setanymousCheck(distro, await checkanymous(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const anymousChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.anymousChecks[distro])
        .map(async (distro) => [distro, await checkanymous(distro, opts)] as const),
    )
    if (anymousChecks.length) {
      setState({ anymousChecks: { ...state.anymousChecks, ...Object.fromEntries(anymousChecks) } })
    }
  }
  ...
  const refreshanymousCheckBackground = (id: string, distro: string) => {
    void checkanymous(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setanymousCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl anymous check failed", { id, distro, message })
      })
  }

  const refreshanymousChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkanymous(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setanymousCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl anymous check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshanymousCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installanymous(name: string) {
      await runJob({ kind: "install-anymous", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslanymous(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "anymous installation failed")
        }
        await refreshanymousCheck(name, { signal: abort.signal })
        expectanymousVersion(state.anymousChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.anymousChecks, distro) : {}),
      })
    },
...
function anymousCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "anymous is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "anymous is installed but could not run",
    }
  }
  return {
    distro,
