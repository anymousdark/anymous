...
let releaseanymousResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectanymousVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectanymousVersion("1.14.35", "1.16.2")).toThrow(
    "anymous update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating anymous", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.anymous/bin/anymous",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, anymousChecks: {} })
})
...
test("ignores stale background anymous checks after removing a WSL server", async () => {
  persistedServers = []
  releaseanymousResolve = undefined
  ...
  await waitFor(() => !!releaseanymousResolve)
  await controller.removeServer("wsl:Debian")
  releaseanymousResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().anymousChecks).toEqual({})
})

test("ignores stale startup anymous checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking anymous", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const anymous: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveanymous: async (distro) => {
      anymous.push(distro)
      return "/home/me/.anymous/bin/anymous"
    },
  })
  ...
  expect(anymous).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(anymous).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().anymousChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check anymous in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const anymous: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(anymous).toEqual(["Debian"])
  expect(Object.keys(controller.getState().anymousChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveanymous: async () => {
      await new Promise<void>((resolve) => {
        releaseanymousResolve = resolve
      })
      return "/home/me/.anymous/bin/anymous"
    },
  }
