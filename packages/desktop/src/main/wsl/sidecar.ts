...
import { type WslCommandLine, resolveWslanymous, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const anymous = await resolveWslanymous(distro)
  if (!anymous) throw new Error(`anymous is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "anymous"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export ANYMOUS_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export ANYMOUS_CLIENT=desktop",
    `export ANYMOUS_SERVER_USERNAME=${shellEscape(username)}`,
    `export ANYMOUS_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(anymous)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
