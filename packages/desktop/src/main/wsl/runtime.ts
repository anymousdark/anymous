...
export async function installWslanymous(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      [
...
export async function resolveWslanymous(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.anymous/bin/anymous" ]; then printf "%s\\n" "$HOME/.anymous/bin/anymous"; fi',
        distro,
        opts,
      )
    ).stdout,
