...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier anymous launch", () => {
    expect(hasExistingAppState([file("anymous.settings")])).toBe(true)
    expect(hasExistingAppState([file("anymous.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("anymous")])).toBe(true)
  })
})
