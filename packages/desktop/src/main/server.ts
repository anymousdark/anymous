...
const SIDECAR_SERVICE_NAME = "anymous server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    ANYMOUS_EXPERIMENTAL_ICON_DISCOVERY: "true",
    ANYMOUS_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`anymous:${password}`).toString("base64")
...
function createSidecarEnv(): Record<string, string> {
  ...
  if (!app.isPackaged) env.ANYMOUS_DISABLE_CHANNEL_DB = "1"
  return env
}
