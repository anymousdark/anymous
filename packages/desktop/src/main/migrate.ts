...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.anymous.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `anymous.settings.dat` is special: it maps to the `anymous.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"anymous.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // anymous.settings.dat → the electron settings store ("anymous.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "anymous.global.dat").
  const storeName = filename === "anymous.settings.dat" ? "anymous.settings" : filename
