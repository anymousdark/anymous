...
import type { UpdaterState } from "@anymous-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@anymous-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.ANYMOUS_CHANNEL === "prod" &&
...
const deepLinkEvent = "anymous:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__ANYMOUS__ ??= {}
  const pending = window.__ANYMOUS__.deepLinks ?? []
  window.__ANYMOUS__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `anymous.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://anymous-cli.vercel.app/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("anymous.global.dat").getItem("language")
