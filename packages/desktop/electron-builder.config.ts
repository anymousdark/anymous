...
const getBase = (appId: string): Configuration => ({
  artifactName: "anymous-desktop-${os}-${arch}.${ext}",
  directories: {
    output: "dist",
    buildResources: "resources",
  },
  // Linux launchers are .desktop files, so this is the desktop file name,
  // not just the app id. For prod, app id "ai.anymous.desktop" becomes
  // "ai.anymous.desktop.desktop".
  ...
  switch (channel) {
    case "dev": {
      return {
        ...base,
        appId,
        productName: "anymous Dev",
        rpm: { packageName: "anymous-dev" },
      }
    }
    case "beta": {
      return {
        ...base,
        appId,
        productName: "anymous Beta",
        protocols: { name: "anymous Beta", schemes: ["anymous"] },
        publish: { provider: "github", owner: "anomalyco", repo: "anymous-beta", channel: "latest" },
        rpm: { packageName: "anymous-beta" },
      }
    }
    case "prod": {
      return {
