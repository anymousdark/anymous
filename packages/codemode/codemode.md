...
This is the living design and status document for `@anymous-ai/codemode` and its existing V2 anymous adapter.
...
`packages/anymous/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
