...
import { ConfigProvider } from "@anymous-ai/core/config/provider"
...
import { Policy } from "@anymous-ai/core/policy"
import type { Project } from "@anymous-ai/core/project"
...
describe("Config", () => {
  ...
  it.live("loads anymous JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "anymous.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "anymous.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "anymous.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "anymous.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "anymous.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "anymous.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .anymous configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".anymous"), { recursive: true })
            await fs.mkdir(path.join(directory, ".anymous"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "anymous.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "anymous.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "anymous.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "anymous.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "anymous.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".anymous", "anymous.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".anymous", "anymous.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".anymous")),
              AbsolutePath.make(path.join(directory, ".anymous")),
            ])
