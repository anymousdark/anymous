...
const anymousClient = process.env.ANYMOUS_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (anymousClient === undefined) delete process.env.ANYMOUS_CLIENT
  else process.env.ANYMOUS_CLIENT = anymousClient
})
...
describe("resource", () => {
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    ...
    expect(resource().attributes["anymous.client"]).toBeDefined()
  })
  ...
  test("keeps built-in attributes when env values conflict", () => {
    ...
    expect(resource().attributes["anymous.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "anymous-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "anymous.log")
