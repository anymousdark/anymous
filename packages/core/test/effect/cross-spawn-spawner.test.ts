...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "anymous-core-test-"))
  return {
    path: dir,
...
describe("cross-spawn spawner", () => {
  ...
  describe("combined output (all)", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        // `echo` is a shell builtin on Windows and would include the argument
        // quotes verbatim in its output, so use node for deterministic output.
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["ANYMOUS_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { ANYMOUS_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("ANYMOUS_TEST_SHELL=ok")
      }),
    )
