...
import { ModelsDevPlugin } from "@anymous-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.ANYMOUS_MODELS_PATH,
          disabled: Flag.ANYMOUS_DISABLE_MODELS_FETCH,
        }
        Flag.ANYMOUS_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.ANYMOUS_MODELS_PATH = previous.path
          Flag.ANYMOUS_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
