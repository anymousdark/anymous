...
import { define } from "@anymous-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@anymous-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "anymous.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "anymous.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
