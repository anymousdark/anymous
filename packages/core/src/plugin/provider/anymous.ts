...
import type { IntegrationOAuthMethodRegistration } from "@anymous-ai/plugin/v2/effect/integration"
import { define } from "@anymous-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@anymous-ai/sdk/v2/types"
...
import type { ConfigProviderV1 } from "../../v1/config/provider"
import { ConfigProviderOptionsV1 } from "../../v1/config/provider-options"
import { ConfigV1 } from "../../v1/config/config"

const defaultServer = "https://anymous-cli.vercel.app"
...
export const anymousPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "anymous",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("anymousPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("anymous")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load anymous provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("anymous", (integration) => {
        integration.name = "anymous"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "anymous", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("anymous")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("anymous")
      ...
      const item = catalog.provider.get(ProviderV2.ID.anymous)
      if (!item) return
      const hasKey = Boolean(process.env.ANYMOUS_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("anymous")),
