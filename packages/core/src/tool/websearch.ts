...
import { ToolFailure } from "@anymous-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@anymous/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ANYMOUS_WEBSEARCH_PROVIDER === "exa" || process.env.ANYMOUS_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ANYMOUS_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ANYMOUS_EXPERIMENTAL") || truthy("ANYMOUS_ENABLE_EXA") || truthy("ANYMOUS_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ANYMOUS_ENABLE_PARALLEL") || truthy("ANYMOUS_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `anymous/${InstallationVersion}`,
