import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@anymous-ai/plugin/tui"
...
import type { useanymousKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useanymousKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getanymousModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getanymousModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
