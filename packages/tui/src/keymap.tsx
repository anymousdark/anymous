...
export const ANYMOUS_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const ANYMOUS_MODE_KEY = "anymous.mode"

export const AnymousKeymapProvider = KeymapProvider
export const anymousKeymapProvider = KeymapProvider
export const useanymousKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type anymousModeStack = ReturnType<typeof createanymousModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, anymousModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createanymousModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(ANYMOUS_MODE_KEY, ANYMOUS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ANYMOUS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ANYMOUS_MODE_KEY, stack.at(-1)?.mode ?? ANYMOUS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ANYMOUS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ANYMOUS_MODE_KEY, undefined)
...
export function useanymousModeStack() {
  return getanymousModeStack(useanymousKeymap())
}

export function getanymousModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("anymous mode stack is not registered for this keymap")
  return value
}
...
export function registeranymousKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createanymousModeStack(keymap)
