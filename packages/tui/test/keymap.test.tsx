...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registeranymousKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when anymous mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getanymousModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getanymousModeStack(keymap).push("autocomplete")
