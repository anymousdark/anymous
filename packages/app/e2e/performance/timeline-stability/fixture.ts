...
import { SessionV1 } from "@anymous-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockanymousServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/anymous/TimelineStability"
...
export const model = { providerID: "anymous", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("anymous.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
