...
const directory = "C:/anymous/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockanymousServer(page, {
    ...
    provider: {
      ...
      connected: ["anymous"],
      default: { providerID: "anymous", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "anymous.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "anymous.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "anymous.window.browser.dat:tabs",
