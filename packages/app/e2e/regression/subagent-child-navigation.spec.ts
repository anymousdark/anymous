...
import { currentSession, mockanymousServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/anymous/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#anymous-titlebar-right")
...
async function setup(page: Page, events?: () => EventPayload[]) {
  await mockanymousServer(page, {
    ...
    provider: {
      ...
      connected: ["anymous"],
      default: { providerID: "anymous", modelID: "claude-opus-4-6" },
    },
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "anymous", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "anymous.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("anymous.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
