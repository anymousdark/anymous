import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@anymous-ai/client/promise"
...
import { List, type ListRef } from "@anymous-ai/ui/list"
import { ProviderIcon } from "@anymous-ai/ui/provider-icon"
import { Spinner } from "@anymous-ai/ui/spinner"
import { Tag } from "@anymous-ai/ui/tag"
import { TextField } from "@anymous-ai/ui/text-field"
import { ButtonV2 } from "@anymous-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@anymous-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@anymous-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "anymous-go") return language.t("dialog.provider.anymousGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "anymous"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.anymous.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["anymous", "anymous-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "anymous" || provider.id === "anymous-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "anymous"
                                ? "dialog.provider.anymous.tagline"
                                : "dialog.provider.anymousGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "anymous"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.anymousZen.line1")}</div>
              <div>{language.t("provider.connect.anymousZen.line2")}</div>
              <div>
                {language.t("provider.connect.anymousZen.visit.prefix")}
                <Link
                  href="https://anymous-cli.vercel.app/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.anymousZen.visit.link")}
                </Link>
                {language.t("provider.connect.anymousZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "anymous"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.anymousZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.anymousZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.anymousZen.visit.prefix")}
                <Link href="https://anymous-cli.vercel.app/zen" tabIndex={-1}>
                  {language.t("provider.connect.anymousZen.visit.link")}
                </Link>
                {language.t("provider.connect.anymousZen.visit.suffix")}
              </div>
