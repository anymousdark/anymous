...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("anymous://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("anymous://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("anymous://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("anymous://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("anymous://open-project")).toBeUndefined()
    expect(parseDeepLink("anymous://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "anymous://open-project?directory=/a",
      "anymous://other?directory=/b",
      "anymous://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("anymous://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("anymous://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("anymous://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("anymous://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "anymous://new-session?directory=/a",
      "anymous://open-project?directory=/b",
      "anymous://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __ANYMOUS__: {
        deepLinks: ["anymous://open-project?directory=/a"],
      },
    } as unknown as Window & { __ANYMOUS__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["anymous://open-project?directory=/a"])
