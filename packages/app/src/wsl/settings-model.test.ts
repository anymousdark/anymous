...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when anymous needs attention", () => {
    expect(wslanymousAction(undefined)).toBeUndefined()
    expect(
      wslanymousAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install anymous")
    expect(
      wslanymousAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/anymous",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when anymous is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        anymousChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.anymous/bin/anymous",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installanymous" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-anymous")
  })
