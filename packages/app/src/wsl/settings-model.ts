...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-anymous" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslanymousAction(check?: WslanymousCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install anymous"
  if (check.matchesDesktop === false) return "Update anymous"
}
...
export function addServerViewModel(input: {
  ...
  const anymousCheck = selectedDistro ? (state?.anymousChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.anymousChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateanymous" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.anymousMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installanymous" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  anymousCheck: WslanymousCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installinganymous =
    input.state?.job?.kind === "install-anymous" && input.state.job.distro === input.selectedDistro
  if (!ready || probinganymous) {
    return {
      variant: "contrast",
  ...
  if (!addServeranymousReady(input.anymousCheck)) {
    const update = !!input.anymousCheck?.resolvedPath && input.anymousCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installinganymous
        ? { key: "wsl.onboarding.updatinganymous" }
        : update
          ? { key: "wsl.onboarding.updateanymous" }
          : { key: "wsl.onboarding.installanymous" },
      disabled: !!input.state?.job || input.adding,
      action: "install-anymous",
      loading: installinganymous,
...
function addServeranymousReady(check: WslanymousCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.anymousChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.anymousChecks[item.name]) return [`anymous:${item.name}`]
    return []
  })
