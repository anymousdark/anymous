...
import type { retry } from "@anymous-ai/core/util/retry"
import type { anymousEvent, SessionApi } from "@anymous-ai/client/promise"
import type { Message, anymousClient, Part, Session } from "@anymous-ai/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as anymousEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as anymousClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
