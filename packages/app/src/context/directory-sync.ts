...
import type { Message, Part, Session } from "@anymous-ai/sdk/v2/client"
...
export const createDirSyncContext = (
  ...
  // NB: child() may throw while the backing store is still bootstrapping
  // (persisted cache not ready). Degrade to undefined instead of crashing.
  const current = createMemo(() => {
    try {
      return serverSync.child(directory, { mcp: true })
    } catch (error) {
      console.warn(`[dir-sync] child store unavailable for ${directory}`, error)
      return undefined
    }
  })
  ...
  const set = ((...input: unknown[]) => {
    ...
    const pair = current()
    if (!pair) return
    const result = (pair[1] as (...args: unknown[]) => unknown)(...input)
    if (input[0] === "session") pair[0]?.session?.forEach(serverSync.session.remember)
    return result
  ...
  return {
    ...
    get ready() {
      return (current()?.[0]?.status ?? "loading") !== "loading"
    },
    get project() {
    ...
    session: {
      ...
      more: createMemo(() => (current()?.[0]?.session.length ?? 0) >= (current()?.[0]?.limit ?? 0)),
    ...
    absolute,
    get directory() {
      return current()?.[0]?.path?.directory ?? directory
    },
  }
}
