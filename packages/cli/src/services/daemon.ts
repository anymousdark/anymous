...
import { createanymousClient } from "@anymous-ai/sdk/v2/client"
import { ServerAuth } from "@anymous-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createanymousClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@anymous/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createanymousClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createanymousClient({ baseUrl: connection.url, headers: connection.headers })
    })
