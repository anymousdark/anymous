...
import { and, Database, eq, isNull, lt, or, sql } from "@anymous-ai/console-core/drizzle/index.js"
import { KeyTable } from "@anymous-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@anymous-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@anymous-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@anymous-ai/console-core/util/date.js"
import { Identifier } from "@anymous-ai/console-core/identifier.js"
import { Billing } from "@anymous-ai/console-core/billing.js"
import { Actor } from "@anymous-ai/console-core/actor.js"
import { WorkspaceTable } from "@anymous-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@anymous-ai/console-core/model.js"
import { Subscription } from "@anymous-ai/console-core/subscription.js"
import { BlackData } from "@anymous-ai/console-core/black.js"
import { UserTable } from "@anymous-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@anymous-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@anymous-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@anymous-ai/console-core/lite.js"
import { Resource } from "@anymous-ai/console-resource"
...
import { Workspace } from "@anymous-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-anymous-session") ?? ""
    const requestId = input.request.headers.get("x-anymous-request") ?? ""
    const ocClient = input.request.headers.get("x-anymous-client") ?? ""
    const projectId = input.request.headers.get("x-anymous-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://anymous-cli.vercel.app/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-anymous-request")
            headers.delete("x-anymous-session")
            headers.delete("x-anymous-project")
            headers.delete("x-anymous-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-anymous-endpoint-id")
        const resEndpointModelId = res.headers.get("x-anymous-upstream-model-id")
    ...
    if (modelData.trialEnded)
      throw new ModelError(
        `${t("zen.api.error.trialEnded", {
          ...
          link: "https://anymous-cli.vercel.app/go",
        })}`,
      )
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://anymous-cli.vercel.app/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://anymous-cli.vercel.app/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://anymous-cli.vercel.app/workspace/${authInfo.workspaceID}/members`
