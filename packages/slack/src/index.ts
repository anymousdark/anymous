...
console.log("🚀 Starting anymous server...")
const anymous = await createanymous({
  port: 0,
})
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new anymous session...")
    const { client, server } = anymous
    ...
    console.log("✅ Created anymous session:", createResult.data.id)
  ...
  console.log("📝 Sending to anymous:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 anymous response:", JSON.stringify(result, null, 2))
