...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve anymous route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "anymous")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "anymous")).toBe("openai")
    expect(statProvider("big-pickle", "", "anymous")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "anymous"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep anymous as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "anymous"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "anymous"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep anymous or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "anymous"), country: "US" })).toMatchObject([
