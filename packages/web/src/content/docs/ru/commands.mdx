...
Создайте `.anymous/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию anymous или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем anymous [config](/docs/config):
...
- Глобальный: `~/.config/anymous/commands/`
- Для каждого проекта: `.anymous/commands/`
...
anymous включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
