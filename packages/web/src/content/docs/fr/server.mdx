---
title: Serveur
description: Interagissez avec le serveur anymous via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `anymous serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client anymous peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `anymous.local` |
...
Définissez `ANYMOUS_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `anymous`, ou définissez `ANYMOUS_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `anymous serve` et à `anymous web`.
...
Lorsque vous exécutez `anymous`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur anymous pour interagir avec anymous par programmation.
:::

Cette architecture permet à anymous de prendre en charge plusieurs clients et vous permet d'interagir avec anymous par programme.

Vous pouvez exécuter `anymous serve` pour démarrer un serveur autonome. Si vous avez le
anymous TUI en cours d'exécution, `anymous serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins anymous [IDE](/docs/ide).
...
Le serveur anymous expose les API suivantes.
