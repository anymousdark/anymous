---
title: SDK
description: Client JS de type sécurisé pour le serveur anymous.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK anymous JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler anymous par programme.
...
Créez une instance de anymous :

```javascript
import { createanymous } from "@anymous-ai/sdk"

const { client } = await createanymous()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `anymous.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${anymous.server.url}`)

anymous.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de anymous, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@anymous-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*anymous" },
})
