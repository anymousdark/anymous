...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez anymous.
...
Voici plusieurs modèles qui fonctionnent bien avec anymous, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration anymous.

```json title="anymous.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [anymous Zen](/docs/zen), vous utiliserez `anymous/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="anymous.jsonc" {7-12,19-24}
{
  "$schema": "https://anymous-cli.vercel.app/config.json",
  "provider": {
    "openai": {
...
```jsonc title="anymous.jsonc" {6-21}
{
  "$schema": "https://anymous-cli.vercel.app/config.json",
  "provider": {
    "anymous": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. anymous est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

anymous est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="anymous.jsonc" {7-18}
{
  "$schema": "https://anymous-cli.vercel.app/config.json",
  "provider": {
    "openai": {
...
Lorsque anymous démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration anymous.
