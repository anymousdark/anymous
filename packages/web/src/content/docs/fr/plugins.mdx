---
title: Plugins
description: Écrivez vos propres plugins pour étendre anymous.
---

Les plugins vous permettent d'étendre anymous en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de anymous.
...
- `.anymous/plugins/` - Plugins au niveau du projet
- `~/.config/anymous/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/anymous/node_modules/`.
...
1. Configuration globale (`~/.config/anymous/anymous.json`)
2. Configuration du projet (`anymous.json`)
3. Répertoire global des plugins (`~/.config/anymous/plugins/`)
4. Répertoire des plugins du projet (`.anymous/plugins/`)
...
anymous exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK anymous pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre anymous.
...
:::note
Si vous utilisez l'application de bureau anymous, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez anymous de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à anymous :
...
L'assistant `tool` crée un outil personnalisé que anymous peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour anymous aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://anymous-cli.vercel.app/docs/sdk) pour plus de détails.
