...
Avec anymous, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, anymous utilise notre propre thème `anymous`.
...
anymous est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que anymous corresponde à l'apparence de leur terminal
...
anymous prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/anymous/themes/*.json` ou `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.anymous/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.anymous/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/anymous/themes
vim ~/.config/anymous/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .anymous/themes
vim .anymous/themes/my-theme.json
```
...
{
  "$schema": "https://anymous-cli.vercel.app/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
