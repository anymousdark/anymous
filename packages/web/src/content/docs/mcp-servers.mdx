...
You can add external tools to anymous using the _Model Context Protocol_, or MCP. anymous supports both local and remote servers.
...
You can define MCP servers in your [anymous Config](https://anymous-cli.vercel.app/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/anymous` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
anymous automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, anymous will:
...
If the server requires authentication, anymous will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `anymous mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, anymous will store the tokens securely in `~/.local/share/anymous/mcp-auth.json`.
...
Your MCPs are available as tools in anymous, alongside built-in tools. So you can manage them through the anymous config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect anymous to your Sentry account.
