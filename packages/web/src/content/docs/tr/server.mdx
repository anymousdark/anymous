---
title: Sunucu
description: anymous sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`anymous serve` komutu, anymous istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `anymous.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `ANYMOUS_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `anymous` değeridir; değiştirmek isterseniz `ANYMOUS_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `anymous serve` hem de `anymous web` için geçerlidir.
...
`anymous` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
anymous ile programatik etkileşim için anymous sunucusunu kullanın.
:::

Bu mimari, anymous'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`anymous serve` ile bağımsız bir sunucu başlatabilirsiniz. anymous TUI açıksa, `anymous serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum anymous [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
anymous sunucusu aşağıdaki API'leri sunar.
