...
_Model Bağlam Protokolü_ veya MCP kullanarak anymous'a harici araçlar ekleyebilirsiniz. anymous hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [anymous Config](https://anymous-cli.vercel.app/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/anymous` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
anymous, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde anymous şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa anymous, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `anymous mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından anymous, belirteçleri `~/.local/share/anymous/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra anymous'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi anymous yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve anymous'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
