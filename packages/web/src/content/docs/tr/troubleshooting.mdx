...
anymous ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\anymous\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `anymous --log-level DEBUG`.

---

## Depolama

anymous, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/anymous/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\anymous` yapistirin
...
anymous Desktop arka planda yerel bir anymous sunucusu (`anymous-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `anymous` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/anymous/anymous.jsonc` (veya `~/.config/anymous/anymous.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/anymous/anymous.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\anymous\anymous.jsonc` yapistirin
...
anymous diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/anymous/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\anymous\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), anymous'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. anymous Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/anymous`
- **Linux**: `~/.cache/anymous` dizinini silin (veya `rm -rf ~/.cache/anymous` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\anymous` yapistirin

3. anymous Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

anymous Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`anymous.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `ANYMOUS_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `ANYMOUS_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta anymous Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, anymous ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

anymous Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda anymous icin bildirimler etkinse
...
1. anymous Desktop'u kapatin
2. Su dosyalari bulun ve silin (anymous Desktop uygulama veri dizininde yer alirlar):

- `anymous.settings.dat` (desktop varsayilan sunucu URL'si)
- `anymous.global.dat` ve `anymous.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
anymous ile ilgili bir sorun yasiyorsaniz:
...
3. `anymous upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `anymous models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\anymous`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. anymous, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\anymous`

2. En guncel provider paketlerini yeniden kurmak icin anymous'u yeniden baslatin

Bu, anymous'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
anymous, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
