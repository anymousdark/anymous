...
description: anymous, LSP sunucularınızla bütünleşir.
---

anymous, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

anymous, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`ANYMOUS_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve anymous bir dosyayı açtığında:
...
LSP sunucularını, anymous yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
