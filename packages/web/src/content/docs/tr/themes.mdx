...
anymous ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak anymous kendi `anymous` temasını kullanır.
...
anymous birden fazla yerleşik temayla gelir.
...
- anymous'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
anymous, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/anymous/themes/*.json` veya `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.anymous/themes/*.json`
4. **Geçerli çalışma dizini** - `./.anymous/themes/*.json`
