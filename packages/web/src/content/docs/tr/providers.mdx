---
title: Sağlayıcılar
description: anymous'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

anymous, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı anymous yapılandırmanızda yapılandırın.
...
`~/.local/share/anymous/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları anymous'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
anymous Zen, anymous ekibi tarafından sağlanan modellerin bir listesidir.
anymous ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz anymous Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, anymous'u seçin ve [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/auth)'ye gidin.
...
anymous'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## anymous Go

anymous Go, anymous ile iyi çalıştığı test edilmiş ve doğrulanmış, anymous ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `anymous Go`'yu seçin ve [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen) adresine gidin.
...
Amazon Bedrock'u anymous ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   anymous'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `anymous.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi anymous'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
anymous'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   anymous'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   anymous yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
anymous, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. anymous'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`anymous.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [anymous-gitlab-auth](https://www.npmjs.com/package/anymous-gitlab-auth) ana sayfasında.

##### Yapılandırma

`anymous.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi anymous'la kullanmak için:
...
Google Vertex AI'yi anymous ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları anymous çalıştırılırken ayarlayın.
...
Helicone'un anymous aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her anymous görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [anymous-helicone-session](https://github.com/H2Shami/anymous-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her anymous görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
anymous'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla anymous'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla anymous'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini anymous için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/anymous)'a bakın.
:::
...
Ollama Cloud'u anymous ile kullanmak için:
...
3. anymous'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: anymous'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
anymous Zen, anymous ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>anymous Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **anymous Zen**'i arayın.

   ```txt
   /connect
   ```

3. anymous API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   anymous yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca anymous yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi anymous ile kullanmak için:
...
Modelleri anymous yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı anymous ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `anymous.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları anymous'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `anymous auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için anymous yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, anymous yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
