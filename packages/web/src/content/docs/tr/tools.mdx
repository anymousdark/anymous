...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. anymous yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
anymous içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `ANYMOUS_EXPERIMENTAL_LSP_TOOL=true` (veya `ANYMOUS_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca anymous provider kullanılırken veya `ANYMOUS_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

anymous başlatırken etkinleştirmek için:
