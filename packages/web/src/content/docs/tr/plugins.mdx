---
title: Eklentiler
description: anymous'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek anymous'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya anymous'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.anymous/plugins/` - Proje düzeyinde eklentiler
- `~/.config/anymous/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/anymous/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/anymous/anymous.json`)
2. Project config (`anymous.json`)
3. Global eklenti dizini (`~/.config/anymous/plugins/`)
4. Proje eklenti dizini (`.anymous/plugins/`)
...
anymous bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir anymous SDK istemcisi.
...
anymous'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
anymous masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
anymous'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca anymous'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, anymous'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra anymous'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://anymous-cli.vercel.app/docs/sdk) bakın.
