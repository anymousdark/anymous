...
anymous Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, anymous'daki diğer sağlayıcılar gibi çalışır. anymous Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve anymous'u kullanmak için bunu kullanmanıza gerek yoktur.
...
anymous Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

anymous Go, anymous'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>anymous Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `anymous Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye anymous Go'ya abone olabilir.
:::
...
anymous Go aşağıdaki limitleri içerir:
...
anymous yapılandırmanızdaki [model id](/docs/config/#models) formatı `anymous-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `anymous-go/kimi-k3` kullanmalısınız.
...
anymous Go'yu şu amaçlarla oluşturduk:
...
4. anymous ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
