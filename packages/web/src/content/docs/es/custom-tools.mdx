...
description: Cree herramientas que LLM pueda llamar en anymous.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de anymous como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.anymous/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/anymous/tools/`.
...
```ts title=".anymous/tools/database.ts" {1}
...
```ts title=".anymous/tools/math.ts"
import { tool } from "@anymous-ai/plugin"
...
```ts title=".anymous/tools/bash.ts"
import { tool } from "@anymous-ai/plugin"
...
```ts title=".anymous/tools/project.ts" {8}
import { tool } from "@anymous-ai/plugin"
...
```python title=".anymous/tools/add.py"
import sys
...
```ts title=".anymous/tools/python-add.ts" {10}
import { tool } from "@anymous-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".anymous/tools/add.py")
