...
Puede agregar herramientas externas a anymous utilizando el _Model Context Protocol_ o MCP. anymous admite servidores locales y remotos.
...
Puede definir servidores MCP en su [anymous Config](https://anymous-cli.vercel.app/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/anymous`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
anymous maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, anymous:
...
Si el servidor requiere autenticación, anymous le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `anymous mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, anymous almacenará los tokens de forma segura en `~/.local/share/anymous/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en anymous, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración anymous como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar anymous a su cuenta Sentry.
