---
title: Complementos
description: Escriba sus propios complementos para extender anymous.
---

Los complementos le permiten extender anymous al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de anymous.
...
- `.anymous/plugins/` - Complementos a nivel de proyecto
- `~/.config/anymous/plugins/` - Complementos globales
...
```json title="anymous.json"
{
  ...
  "plugin": ["anymous-helicone-session", "anymous-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/anymous/node_modules/`.
...
1. Configuración global (`~/.config/anymous/anymous.json`)
2. Configuración del proyecto (`anymous.json`)
3. Directorio global de complementos (`~/.config/anymous/plugins/`)
4. Directorio de complementos del proyecto (`.anymous/plugins/`)
...
```json title=".anymous/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
anymous ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".anymous/plugins/my-plugin.ts"
...
```js title=".anymous/plugins/example.js"
...
- `client`: Un cliente SDK anymous para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar anymous.
...
```js title=".anymous/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "anymous"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio anymous, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que anymous lea archivos `.env`:

```javascript title=".anymous/plugins/env-protection.js"
...
```javascript title=".anymous/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a anymous:

```ts title=".anymous/plugins/custom-tools.ts"
import { type Plugin, tool } from "@anymous-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que anymous puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para anymous junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://anymous-cli.vercel.app/docs/sdk) para obtener más detalles.
...
```ts title=".anymous/plugins/compaction.ts"
import type { Plugin } from "@anymous-ai/plugin"
...
```ts title=".anymous/plugins/custom-compaction.ts"
import type { Plugin } from "@anymous-ai/plugin"
