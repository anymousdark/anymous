...
Para depurar problemas con anymous, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/anymous/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\anymous\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `anymous --log-level DEBUG`.

---

## Almacenamiento

anymous almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/anymous/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\anymous`
...
anymous Desktop ejecuta un servidor anymous local (el sidecar `anymous-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `anymous` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/anymous/anymous.jsonc` (o `~/.config/anymous/anymous.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/anymous/anymous.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\anymous\anymous.jsonc`
...
anymous también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/anymous/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\anymous\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.anymous/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que anymous pueda reconstruirlo.

1. Salga de anymous Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/anymous`
- **Linux**: eliminar `~/.cache/anymous` (o ejecutar `rm -rf ~/.cache/anymous`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\anymous`

3. Reinicie el escritorio anymous.

---

### Solucionar problemas de conexión del servidor

anymous Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `anymous.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `ANYMOUS_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `ANYMOUS_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio anymous requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de anymous.

---

### Notificaciones que no se muestran

anymous Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para anymous en la configuración de su sistema operativo, y
...
1. Salga del escritorio anymous.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio anymous):

- `anymous.settings.dat` (URL del servidor predeterminado de escritorio)
- `anymous.global.dat` y `anymous.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con anymous:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anymousdark/anymous/issues**](https://github.com/anymousdark/anymous/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**anymous-cli.vercel.app/discord**](https://anymous-cli.vercel.app/discord)
...
3. Asegúrese de tener la última versión con `anymous upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `anymous models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/anymous
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\anymous`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. anymous instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\anymous`

2. Reinicie anymous para reinstalar los paquetes del proveedor más recientes.

Esto obligará a anymous a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
anymous detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
