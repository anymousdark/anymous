---
title: Servidor
description: Interactuar con el servidor anymous a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `anymous serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente anymous puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `anymous.local` |
...
Configure `ANYMOUS_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `anymous`, o configure `ANYMOUS_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `anymous serve` como a `anymous web`.
...
Cuando ejecuta `anymous`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor anymous para interactuar con anymous mediante programación.
:::

Esta arquitectura permite que anymous admita múltiples clientes y le permite interactuar con anymous mediante programación.

Puede ejecutar `anymous serve` para iniciar un servidor independiente. Si tienes el
anymous TUI ejecutándose, `anymous serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos anymous [IDE](/docs/ide).
...
El servidor anymous expone las siguientes API.
