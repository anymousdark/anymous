...
Las habilidades del agente permiten a anymous descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
anymous busca estas ubicaciones:

- Configuración del proyecto: `.anymous/skills/<name>/SKILL.md`
- Configuración global: `~/.config/anymous/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, anymous sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.anymous/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/anymous/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.anymous/skills/git-release/SKILL.md` así:
...
anymous enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `anymous.json`:
...
**Para agentes integrados** (en `anymous.json`):
