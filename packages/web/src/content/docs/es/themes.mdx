...
Con anymous puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, anymous usa nuestro propio tema `anymous`.
...
anymous viene con varios temas integrados.
...
- Quiere que anymous coincida con la apariencia de su terminal
...
anymous admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/anymous/themes/*.json` o `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.anymous/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.anymous/themes/*.json`
