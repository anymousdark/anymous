...
Las herramientas permiten que LLM realice acciones en su código base. anymous viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en anymous.
...
:::note
Esta herramienta solo está disponible cuando `ANYMOUS_EXPERIMENTAL_LSP_TOOL=true` (o `ANYMOUS_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor anymous o cuando la variable de entorno `ANYMOUS_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar anymous:

```bash
ANYMOUS_ENABLE_EXA=1 anymous
```
