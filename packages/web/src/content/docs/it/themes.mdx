...
Con anymous puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, anymous usa il tema `anymous`.
...
anymous include diversi temi integrati.
...
- Vuole che anymous corrisponda all'aspetto del terminale
...
anymous supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/anymous/themes/*.json` o `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.anymous/themes/*.json`
4. **Directory di lavoro corrente** - in `./.anymous/themes/*.json`
