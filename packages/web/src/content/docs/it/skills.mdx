...
Le skill degli agenti permettono a anymous di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
anymous cerca in queste posizioni:

- Config di progetto: `.anymous/skills/<name>/SKILL.md`
- Config globale: `~/.config/anymous/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, anymous risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.anymous/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/anymous/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.anymous/skills/git-release/SKILL.md` cosi':
...
anymous elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `anymous.json`:
...
**Per agenti integrati** (in `anymous.json`):
