...
anymous Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in anymous. Ti abboni a anymous Go e
...
anymous Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

anymous Go funziona come qualsiasi altro provider in anymous.

1. Accedi a **<a href={console}>anymous Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `anymous Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a anymous Go.
:::
...
anymous Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua anymous config
utilizza il formato `anymous-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`anymous-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con anymous.
