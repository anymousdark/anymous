---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di anymous.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

anymous fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire anymous avvia la TUI per la directory corrente.
...
Quando usi la TUI di anymous, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a anymous. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da anymous. _Alias_: `/quit`, `/q`
...
Questo è separato da `anymous.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo anymous attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `anymous.default`.
