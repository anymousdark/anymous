...
Puoi aggiungere strumenti esterni a anymous usando il _Model Context Protocol_, o MCP. anymous supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [anymous Config](https://anymous-cli.vercel.app/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/anymous`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
anymous gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, anymous:
...
Se il server richiede autenticazione, anymous ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `anymous mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, anymous memorizzera' i token in modo sicuro in `~/.local/share/anymous/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in anymous insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione anymous come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare anymous al tuo account Sentry.
