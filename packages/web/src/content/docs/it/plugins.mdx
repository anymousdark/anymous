---
title: Plugin
description: Scrivi plugin per estendere anymous.
---

I plugin ti permettono di estendere anymous agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di anymous.
...
- `.anymous/plugins/` - plugin a livello progetto
- `~/.config/anymous/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/anymous/node_modules/`.
...
1. Config globale (`~/.config/anymous/anymous.json`)
2. Config di progetto (`anymous.json`)
3. Directory plugin globale (`~/.config/anymous/plugins/`)
4. Directory plugin di progetto (`.anymous/plugins/`)
...
anymous esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di anymous per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere anymous.
...
:::note
Se usi l'app desktop di anymous, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a anymous di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a anymous:
...
L'helper `tool` crea uno strumento personalizzato che anymous può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in anymous insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://anymous-cli.vercel.app/docs/sdk) per i dettagli.
