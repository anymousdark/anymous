---
title: Server
description: Interagisci con il server anymous via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `anymous serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client anymous.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `anymous.local` |
...
Imposta `ANYMOUS_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `anymous`, oppure imposta `ANYMOUS_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `anymous serve` sia per `anymous web`.
...
Quando esegui `anymous` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server anymous per interagire con anymous in modo programmatico.
:::

Questa architettura permette a anymous di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `anymous serve` per avviare un server standalone. Se la TUI di anymous e gia in esecuzione, `anymous serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di anymous.
...
Il server anymous espone le seguenti API.
