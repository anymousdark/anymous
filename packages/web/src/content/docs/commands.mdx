...
Create `.anymous/commands/test.md`:
...
You can add custom commands through the anymous config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your anymous [config](/docs/config):
...
anymous includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
