...
أنشئ الملف `.anymous/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات anymous أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ anymous:
...
- على مستوى النظام: `~/.config/anymous/commands/`
- لكل مشروع: `.anymous/commands/`
...
يتضمن anymous عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
