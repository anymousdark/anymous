---
title: المزوّدون
description: استخدام أي موفّر LLM في anymous.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم anymous ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات anymous.
...
في `~/.local/share/anymous/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات anymous.
...
anymous Zen هي قائمة نماذج يوفّرها فريق anymous وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع anymous. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع anymous Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر anymous، ثم انتقل إلى [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/auth).
...
يعمل مثل أي مزوّد آخر في anymous واستخدامه اختياري بالكامل.

---

## anymous Go

anymous Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق anymous، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع anymous.

1. شغّل الأمر `/connect` في TUI، واختر `anymous Go`، ثم انتقل إلى [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen).
...
لاستخدام Amazon Bedrock مع anymous:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل anymous:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `anymous.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل anymous غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين anymous لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل anymous بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات anymous.
...
تخزين الرمز في مساحة تخزين مصادقة anymous.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم anymous نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر anymous
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`anymous.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [anymous-gitlab-auth](https://www.npmjs.com/package/anymous-gitlab-auth).

##### التهيئة

خصّص عبر `anymous.json`:
...
لاستخدام اشتراك GitHub Copilot مع anymous:
...
لاستخدام Google Vertex AI مع anymous:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر anymous، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [anymous-helicone-session](https://github.com/H2Shami/anymous-helicone-session) لتسجيل كل محادثة anymous تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة anymous مدرجة كجلسة مستقلة.
...
يمكنك ضبط anymous لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط anymous لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط anymous لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع anymous. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/anymous) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع anymous:
...
3. انسخ مفتاح API لاستخدامه في anymous.
...
6. **مهم**: قبل استخدام نماذج السحابة في anymous، يجب سحب معلومات النموذج محليا:
...
anymous Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق anymous. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>anymous Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **anymous Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ anymous.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات anymous.
...
5. يمكنك أيضا تخصيصها عبر إعدادات anymous. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع anymous:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات anymous. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع anymous. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `anymous.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ anymous بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `anymous auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات anymous وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات anymous.
