...
تتيح مهارات الوكيل لـ anymous اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث anymous في هذه المواقع:

- إعدادات المشروع: `.anymous/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/anymous/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج anymous صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.anymous/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/anymous/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.anymous/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض anymous المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `anymous.json`:
...
**للوكلاء المدمجين** (في `anymous.json`):
