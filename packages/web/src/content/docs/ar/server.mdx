---
title: الخادم
description: تفاعل مع خادم anymous عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `anymous serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل anymous استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `anymous.local` |
...
عيّن `ANYMOUS_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `anymous`، أو عيّن `ANYMOUS_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `anymous serve` و `anymous web`.
...
عند تشغيل `anymous` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم anymous للتفاعل مع anymous برمجيا.
:::

تتيح هذه البنية لـ anymous دعم عدة عملاء وتمكّنك من التفاعل مع anymous برمجيا.

يمكنك تشغيل `anymous serve` لبدء خادم مستقل. إذا كانت واجهة anymous في terminal (TUI)
قيد التشغيل، فسيبدأ `anymous serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات anymous لـ [IDE](/docs/ide).
...
يعرض خادم anymous واجهات API التالية.
