...
يمكنك إضافة أدوات خارجية إلى anymous باستخدام _Model Context Protocol_، أو MCP. يدعم anymous خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات anymous](https://anymous-cli.vercel.app/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/anymous` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى anymous تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم anymous بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك anymous المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `anymous mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن anymous الرموز بشكل آمن في `~/.local/share/anymous/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل anymous إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات anymous مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط anymous بحساب Sentry الخاص بك.
