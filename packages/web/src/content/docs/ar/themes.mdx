...
مع anymous يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم anymous سمتنا `anymous`.
...
يأتي anymous مع عدة سمات مدمجة.
...
- يريدون أن يطابق anymous مظهر terminal لديهم
...
يدعم anymous نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/anymous/themes/*.json` أو `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.anymous/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.anymous/themes/*.json`
