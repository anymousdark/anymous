---
title: Server
description: Interact with anymous server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `anymous serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an anymous client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `anymous.local` |
...
Set `ANYMOUS_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `anymous`, or set `ANYMOUS_SERVER_USERNAME` to override it. This applies to both `anymous serve` and `anymous web`.
...
When you run `anymous` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the anymous server to interact with anymous programmatically.
:::

This architecture lets anymous support multiple clients and allows you to interact with anymous programmatically.

You can run `anymous serve` to start a standalone server. If you have the
anymous TUI running, `anymous serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the anymous [IDE](/docs/ide) plugins.
...
The anymous server exposes the following APIs.
