---
title: Config
description: Using the anymous JSON config.
---

You can configure anymous using a JSON config file.

---

## Format

anymous supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="anymous.jsonc"
{
...
1. **Remote config** (from `.well-known/anymous`) - organizational defaults
2. **Global config** (`~/.config/anymous/anymous.json`) - user preferences
3. **Custom config** (`ANYMOUS_CONFIG` env var) - custom overrides
4. **Project config** (`anymous.json` in project) - project-specific settings
5. **`.anymous` directories** - agents, commands, plugins
6. **Inline config** (`ANYMOUS_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/anymous/` on macOS) - admin-controlled
...
:::note
The `.anymous` and `~/.config/anymous` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/anymous` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/anymous"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global anymous config in `~/.config/anymous/anymous.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/anymous/tui.json`.
...
Add `anymous.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When anymous starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `ANYMOUS_CONFIG` environment variable.

```bash
export ANYMOUS_CONFIG=/path/to/my/custom-config.json
anymous run "Hello world"
```
...
Specify a custom config directory using the `ANYMOUS_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.anymous` directory, and should
follow the same structure.

```bash
export ANYMOUS_CONFIG_DIR=/path/to/my/config-directory
anymous run "Hello world"
```

The custom directory is loaded after the global config and `.anymous` directories, so it **can override** their settings.
...
Drop an `anymous.json` or `anymous.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/anymous/` |
| Linux    | `/etc/anymous/`                         |
| Windows  | `%ProgramData%\anymous`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, anymous reads managed preferences from the `ai.anymous.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

anymous checks these paths:

1. `/Library/Managed Preferences/<user>/ai.anymous.managed.plist`
2. `/Library/Managed Preferences/ai.anymous.managed.plist`

The plist keys map directly to `anymous.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.anymous.managed` PayloadType. The anymous config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.anymous.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.anymous.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.anymous</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`anymous-cli.vercel.app/config.json`**](https://anymous-cli.vercel.app/config.json).

TUI config uses [**`anymous-cli.vercel.app/tui.json`**](https://anymous-cli.vercel.app/tui.json).
...
Use `ANYMOUS_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `anymous.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `anymous serve` and `anymous web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your anymous server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `anymous.local`. Useful for running multiple instances on the same network.
...
If not specified, anymous will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your anymous config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, anymous tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny anymous actions on configured resources. Currently, policies can control which providers anymous may use.
...
anymous normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, anymous omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/anymous/agents/` or `.anymous/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, anymous will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`anymous run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/anymous/commands/` or `.anymous/commands/`. [Learn more here](/docs/commands).
...
anymous uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
anymous will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, anymous **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend anymous with custom tools, hooks, and integrations.

Place plugin files in `.anymous/plugins/` or `~/.config/anymous/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="anymous.json"
{
  "$schema": "https://anymous-cli.vercel.app/config.json",
  "plugin": ["anymous-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict anymous to only use specific providers rather than disabling them one by one.
