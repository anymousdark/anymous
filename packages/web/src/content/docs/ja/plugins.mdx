---
title: プラグイン
description: anymous を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで anymous を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、anymous のデフォルトの動作を変更したりできます。
...
- `.anymous/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/anymous/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/anymous/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/anymous/anymous.json`)
2. プロジェクト設定 (`anymous.json`)
3. グローバルプラグインディレクトリ (`~/.config/anymous/plugins/`)
4. プロジェクトプラグインディレクトリ (`.anymous/plugins/`)
...
anymous は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための anymous SDK クライアント。
...
ここでは、anymous を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://anymous-cli.vercel.app/docs/sdk) を参照してください。
...
anymous が `.env` ファイルを読み取らないようにします。
...
プラグインは anymous にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、anymous が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して anymous で使用できます。
