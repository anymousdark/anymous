---
title: Policies
description: Control which configured resources anymous may use.
---

Policies control whether anymous may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `anymous.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether anymous may use a resource such as an LLM provider.
...
anymous currently supports one policy action:
