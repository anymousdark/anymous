---
title: Konfiguracija
description: Korištenje anymous JSON konfiguracije.
---

Možete konfigurirati anymous koristeći JSON konfiguracijski fajl.

---

## Format

anymous podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/anymous`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/anymous/anymous.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`ANYMOUS_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`anymous.json` u projektu) - postavke specifične za projekat
5. **`.anymous` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`ANYMOUS_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.anymous` i `~/.config/anymous` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/anymous` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu anymous konfiguraciju u `~/.config/anymous/anymous.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/anymous/tui.json`.
...
Dodajte `anymous.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se anymous pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `ANYMOUS_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `ANYMOUS_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.anymous` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.anymous`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`anymous-cli.vercel.app/config.json`**](https://anymous-cli.vercel.app/config.json).

TUI konfiguracija koristi [**`anymous-cli.vercel.app/tui.json`**](https://anymous-cli.vercel.app/tui.json).
...
Koristite `ANYMOUS_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `anymous.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `anymous serve` i `anymous web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš anymous server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `anymous.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj anymous konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, anymous pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/anymous/agents/` ili `.anymous/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, anymous će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`anymous run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/anymous/commands/` ili `.anymous/commands/`. [Saznajte više ovdje](/docs/commands).
...
anymous će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, anymous **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju anymous sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.anymous/plugins/` ili `~/.config/anymous/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite anymous da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
