...
Agent skills omogucavaju anymous da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
anymous pretrazuje ove lokacije:

- Konfiguracija projekta: `.anymous/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/anymous/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, anymous ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.anymous/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/anymous/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.anymous/skills/git-release/SKILL.md` ovako:
...
anymous navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `anymous.json`:
...
**Za ugradene agente** (u `anymous.json`):
