...
U anymous mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, anymous koristi nasu `anymous` temu.
...
anymous dolazi sa vise ugradenih tema.
...
- Zele da anymous odgovara izgledu njihovog terminala
...
anymous podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/anymous/themes/*.json` ili `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.anymous/themes/*.json`
4. **Trenutni radni direktorij** - `./.anymous/themes/*.json`
