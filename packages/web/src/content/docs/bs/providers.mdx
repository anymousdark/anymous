---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u anymous.
---

import config from "../../../../config.mjs"
export const console = config.console

anymous koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj anymous konfiguraciji.
...
u `~/.local/share/anymous/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem anymous
config.
...
anymous Zen je lista modela koje je obezbedio anymous tim koji su bili
testirano i potvrđeno da dobro radi sa anymous. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa anymous Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite anymous i idite na [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/auth).
...
Radi kao i svaki drugi provajder u anymous i potpuno je opcionalan za korištenje.

---

## anymous Go

anymous Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža anymous tim i koji su testirani i verificirani da dobro rade s anymous-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `anymous Go` i idite na [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen).
...
Da biste koristili Amazon Bedrock s anymous:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete anymous:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `anymous.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u anymous.
:::
...
Možete konfigurirati anymous za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi anymous ispravno radio.
   :::
...
Također možete dodati modele kroz svoju anymous konfiguraciju.
...
da pohrani token u anymous auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
anymous koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate anymous
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`anymous.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [anymous-gitlab-auth](https://www.npmjs.com/package/anymous-gitlab-auth).

##### Konfiguracija

Prilagodite putem `anymous.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s anymous:
...
Za korištenje Google Vertex AI s anymous:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće anymous.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem anymousa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [anymous-helicone-session](https://github.com/H2Shami/anymous-helicone-session) da automatski prijavite svaki anymous razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki anymous razgovor naveden kao zasebna sesija.
...
Možete konfigurirati anymous za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati anymous za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati anymous za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za anymous. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/anymous) za detalje.
:::
...
Da biste koristili Ollama Cloud s anymous:
...
3. Kopirajte API ključ za korištenje u anymous.
...
6. **Važno**: Prije upotrebe modela oblaka u anymous, morate lokalno povući informacije o modelu:
...
anymous Zen je lista testiranih i verifikovanih modela koju je obezbedio anymous tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>anymous Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **anymous Zen**.

```txt
   /connect
```

3. Unesite svoj anymous API ključ.
...
Također možete dodati dodatne modele putem vaše anymous konfiguracije.
...
5. Također ih možete prilagoditi putem vaše anymous konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa anymousom:
...
Također možete prilagoditi modele kroz svoju anymous konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s anymous-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `anymous.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju anymous da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `anymous auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite anymous konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj anymous konfiguraciji.
