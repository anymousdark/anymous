...
Možete dodati vanjske alate u anymous koristeći _Model Context Protocol_, ili MCP. anymous podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [anymous Config](https://anymous-cli.vercel.app/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/anymous`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
anymous automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, anymous će:
...
Ako server zahtijeva autentifikaciju, anymous će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `anymous mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, anymous sigurno čuva tokene u `~/.local/share/anymous/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u anymous, zajedno s ugrađenim alatima. Možete njima upravljati kroz anymous konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete anymous sa vašim Sentry nalogom.
