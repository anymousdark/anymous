...
Postoje dvije vrste agenata u anymous: primarni agenti i podagenti.
...
anymous dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
anymous dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

anymous dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u anymous-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `anymous.json`:
...
- Globalno: `~/.config/anymous/agents/`
- Po projektu: `.anymous/agents/`

```markdown title="~/.config/anymous/agents/review.md"
---
...
Ako temperatura nije navedena, anymous koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu anymous konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj anymous konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [anymous Zen](/docs/zen), koristili biste `anymous/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="anymous.json" {3-6,9-12}
{
  "$schema": "https://anymous-cli.vercel.app/config.json",
  "tools": {
    "write": true,
...
```json title="anymous.json" {3-5,8-10}
{
  "$schema": "https://anymous-cli.vercel.app/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `anymous models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anymousdark/anymous).
:::
...
```markdown title="~/.config/anymous/agents/docs-writer.md"
---
...
```markdown title="~/.config/anymous/agents/security-auditor.md"
---
