---
title: Server
description: Komunicirajte s anymous serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `anymous serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji anymous klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `anymous.local` |
...
Postavite `ANYMOUS_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `anymous`, ili postavite `ANYMOUS_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `anymous serve` i za `anymous web`.
...
Kada pokrenete `anymous`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite anymous server za programsku interakciju sa anymous-om.
:::

Ova arhitektura omogucava anymous podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `anymous serve` da startate standalone server. Ako je anymous TUI vec pokrenut, `anymous serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste anymous [IDE](/docs/ide) pluginovi.
...
anymous server izlaže sljedece API-je.
