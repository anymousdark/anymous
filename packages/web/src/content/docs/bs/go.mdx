...
anymous Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u anymous-u. Pretplatite se na anymous Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili anymous.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa anymous-om.
:::
...
anymous Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

anymous Go radi kao bilo koji drugi provajder u anymous-u.

1. Prijavite se na **<a href={console}>anymous Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `anymous Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na anymous Go.
:::
...
anymous Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj anymous konfiguraciji
koristi format `anymous-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`anymous-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo anymous Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz anymous koristite i bilo kojeg drugog provajdera.
