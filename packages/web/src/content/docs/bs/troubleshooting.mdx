...
Da biste otklonili probleme s anymous, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\anymous\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `anymous --log-level DEBUG`.

---

## Pohrana

anymous pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/anymous/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\anymous`
...
anymous Desktop pokreće lokalni anymous server (`anymous-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `anymous` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/anymous/anymous.jsonc` (ili `~/.config/anymous/anymous.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/anymous/anymous.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\anymous\anymous.jsonc`
...
anymous također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/anymous/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\anymous\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga anymous mogao ponovo izgraditi.

1. Potpuno zatvorite anymous Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/anymous`
- **Linux**: obrišite `~/.cache/anymous` (ili pokrenite `rm -rf ~/.cache/anymous`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\anymous`

3. Ponovo pokrenite anymous Desktop.

---

### Rješavanje problema sa vezom na serveru

anymous Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `anymous.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `ANYMOUS_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `ANYMOUS_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, anymous Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa anymous karakteristikama.

---

### Obavještenja se ne prikazuju

anymous Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za anymous u postavkama vašeg OS-a, i
...
1. Zatvorite anymous Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka anymous Desktop aplikacije):

- `anymous.settings.dat` (URL zadanog servera za desktop)
- `anymous.global.dat` i `anymous.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `anymous models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\anymous`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. anymous dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\anymous`

2. Ponovo pokrenite anymous da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti anymous da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
anymous će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
