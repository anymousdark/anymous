---
title: Dodaci
description: Napišite vlastite dodatke za proširenje anymous.
---

Dodaci vam omogućavaju da proširite anymous spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja anymous.
...
- `.anymous/plugins/` - Dodaci na nivou projekta
- `~/.config/anymous/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/anymous/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/anymous/anymous.json`)
2. Konfiguracija projekta (`anymous.json`)
3. Globalni direktorij dodataka (`~/.config/anymous/plugins/`)
4. Direktorij dodataka projekta (`.anymous/plugins/`)
...
anymous pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: anymous SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje anymous.
...
Spriječite anymous da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u anymous:
...
Pomoćnik `tool` kreira prilagođeni alat koji anymous može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za anymous zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://anymous-cli.vercel.app/docs/sdk) za detalje.
