...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. anymous dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u anymous.
...
:::note
Ovaj alat je dostupan samo kada je `ANYMOUS_EXPERIMENTAL_LSP_TOOL=true` (ili `ANYMOUS_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz anymous provajdera ili kada je varijabla `ANYMOUS_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju anymous:
