---
title: Servidor
description: Interaja com o servidor anymous via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `anymous serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente anymous pode usar.
...
```bash
anymous serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `anymous.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
anymous serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `ANYMOUS_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `anymous`, ou defina `ANYMOUS_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `anymous serve` quanto ao `anymous web`.

```bash
ANYMOUS_SERVER_PASSWORD=your-password anymous serve
```
...
Quando você executa `anymous`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor anymous para interagir com o anymous programaticamente.
:::

Essa arquitetura permite que o anymous suporte múltiplos clientes e permite que você interaja com o anymous programaticamente.

Você pode executar `anymous serve` para iniciar um servidor autônomo. Se você tiver o TUI do anymous em execução, `anymous serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do anymous [IDE](/docs/ide).
...
O servidor anymous expõe as seguintes APIs.
