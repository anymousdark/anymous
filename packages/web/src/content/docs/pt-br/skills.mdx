...
As habilidades do agente permitem que o anymous descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O anymous pesquisa nesses locais:

- Configuração do projeto: `.anymous/skills/<name>/SKILL.md`
- Configuração global: `~/.config/anymous/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o anymous sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.anymous/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/anymous/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.anymous/skills/git-release/SKILL.md` assim:
...
O anymous lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `anymous.json`:
...
**Para agentes embutidos** (em `anymous.json`):
