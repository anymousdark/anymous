...
O anymous Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no anymous. Você assina o anymous Go e
...
O anymous Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O anymous Go funciona como qualquer outro provedor no anymous.

1. Você entra no **<a href={console}>anymous Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `anymous Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o anymous Go.
:::
...
O anymous Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do anymous
usa o formato `anymous-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`anymous-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o anymous também.
