---
title: Plugins
description: Escreva seus próprios plugins para estender o anymous.
---

Plugins permitem que você estenda o anymous conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do anymous.
...
- `.anymous/plugins/` - Plugins em nível de projeto
- `~/.config/anymous/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/anymous/node_modules/`.
...
1. Configuração global (`~/.config/anymous/anymous.json`)
2. Configuração do projeto (`anymous.json`)
3. Diretório de plugins global (`~/.config/anymous/plugins/`)
4. Diretório de plugins do projeto (`.anymous/plugins/`)
...
O anymous executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do anymous para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o anymous.
...
:::note
Se você estiver usando o aplicativo desktop anymous, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o anymous de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao anymous:
...
O helper `tool` cria uma ferramenta personalizada que o anymous pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o anymous junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://anymous-cli.vercel.app/docs/sdk) para detalhes.
