...
Existem dois tipos de agentes no anymous; agentes primários e subagentes.
...
anymous vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
anymous vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

anymous vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do anymous, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `anymous.json`:
...
- Global: `~/.config/anymous/agents/`
- Por projeto: `.anymous/agents/`
...
Se nenhuma temperatura for especificada, o anymous usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do anymous quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do anymous usa o formato `provider/model-id`. Por exemplo, se você estiver usando [anymous Zen](/docs/zen), você usaria `anymous/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `anymous models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anymousdark/anymous).
:::
