...
Com o anymous, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o anymous usa nosso próprio tema `anymous`.
...
O anymous vem com vários temas embutidos.
...
- Querem que o anymous corresponda à aparência do seu terminal
...
O anymous suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/anymous/themes/*.json` ou `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.anymous/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.anymous/themes/*.json`
