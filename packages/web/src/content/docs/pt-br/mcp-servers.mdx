...
Você pode adicionar ferramentas externas ao anymous usando o _Model Context Protocol_, ou MCP. O anymous suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do anymous](https://anymous-cli.vercel.app/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/anymous`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O anymous lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o anymous irá:
...
Se o servidor requer autenticação, o anymous solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `anymous mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o anymous armazenará os tokens de forma segura em `~/.local/share/anymous/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no anymous, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do anymous como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o anymous à sua conta do Sentry.
