---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i anymous.
---

import config from "../../../../config.mjs"
export const console = config.console

anymous bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i anymous-konfigurasjonen.
...
i `~/.local/share/anymous/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i anymous
konfig.
...
anymous Zen er en liste over modeller levert av anymous-teamet som har vært
testet og verifisert for å fungere godt med anymous. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med anymous Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `anymous Zen` og gå til [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen).
...
Det fungerer som alle andre leverandører i anymous og er helt valgfritt å bruke.

---

## anymous Go

anymous Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av anymous-teamet som har vært
testet og verifisert for å fungere godt med anymous.

1. Kjør kommandoen `/connect` i TUI, velg `anymous Go`, og gå til [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen).
...
Slik bruker du Amazon Bedrock med anymous:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører anymous:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `anymous.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i anymous støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere anymous til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at anymous skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom anymous-konfigurasjonen.
...
for å lagre token i anymous auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
anymous bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse anymous
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`anymous.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [anymous-gitlab-auth](https://www.npmjs.com/package/anymous-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `anymous.json`:
...
Slik bruker du GitHub Copilot-abonnementet med anymous:
...
Slik bruker du Google Vertex AI med anymous:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører anymous.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom anymous, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [anymous-helicone-session](https://github.com/H2Shami/anymous-helicone-session) plugin for automatisk å logge hver anymous-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver anymous-samtale oppført som en separat økt.
...
Du kan konfigurere anymous for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere anymous for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere anymous for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for anymous. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/anymous) for detaljer.
:::
...
Slik bruker du Ollama Cloud med anymous:
...
3. Kopier API-nøkkelen for bruk i anymous.
...
6. **Viktig**: Før du bruker skymodeller i anymous, må du hente modellinformasjonen lokalt:
...
anymous Zen er en liste over testede og verifiserte modeller levert av anymous-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>anymous Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **anymous Zen**.

   ```txt
   /connect
   ```

3. Skriv inn anymous API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom anymous-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom anymous-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med anymous:
...
Du kan også tilpasse modeller gjennom anymous-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med anymous. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `anymous.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar anymous forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `anymous auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk anymous-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i anymous-konfigurasjonen.
