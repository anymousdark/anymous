...
description: Skriv dine egne programtillegg for å utvide anymous.
---

Plugins lar deg utvide anymous ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til anymous.
...
- `.anymous/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/anymous/node_modules/`.
...
1. Global konfigurasjon (`~/.config/anymous/anymous.json`)
2. Prosjektkonfigurasjon (`anymous.json`)
3. Global plugin-katalog (`~/.config/anymous/plugins/`)
4. Prosjektpluginkatalog (`.anymous/plugins/`)
...
anymous kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En anymous SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide anymous.
...
:::note
Hvis du bruker anymous-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre anymous fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til anymous:
...
`tool`-hjelperen lager et tilpasset verktøy som anymous kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for anymous sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://anymous-cli.vercel.app/docs/sdk) for detaljer.
