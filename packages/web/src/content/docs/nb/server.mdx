---
title: Tjener
description: Samhandle med anymous-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `anymous serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en anymous-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `anymous.local` |
...
Sett `ANYMOUS_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `anymous` som standard, eller sett `ANYMOUS_SERVER_USERNAME` for å overstyre det. Dette gjelder både `anymous serve` og `anymous web`.
...
Når du kjører `anymous` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk anymous-serveren til å samhandle med anymous programmatisk.
:::

Denne arkitekturen lar anymous støtte flere klienter og lar deg samhandle med anymous programmatisk.

Du kan kjøre `anymous serve` for å starte en frittstående server. Hvis du har
anymous TUI kjører, vil `anymous serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av anymous [IDE](/docs/ide) plugins.
...
anymous-serveren viser følgende APIer.
