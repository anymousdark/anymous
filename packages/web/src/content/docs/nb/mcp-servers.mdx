...
Du kan legge til eksterne verktøy i anymous ved å bruke _Model Context Protocol_ eller MCP. anymous støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [anymous Config](https://anymous-cli.vercel.app/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="anymous.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/anymous`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
anymous håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil anymous:
...
Hvis serveren krever autentisering, vil anymous be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `anymous mcp auth <server-name>`.
...
```bash
anymous mcp auth my-oauth-server
```
...
```bash
anymous mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil anymous lagre tokens sikkert i `~/.local/share/anymous/mcp-auth.json`.
...
anymous mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i anymous, sammen med innebygde verktøy. Så du kan administrere dem gjennom anymous-konfigurasjonen som alle andre verktøy.
...
```json title="anymous.json" {11, 14-18}
{
  "$schema": "https://anymous-cli.vercel.app/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
anymous mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble anymous til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver anymous-samtale oppført som en separat økt.
