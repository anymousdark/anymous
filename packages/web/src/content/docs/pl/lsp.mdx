---
title: Serwery LSP
description: anymous integruje się z serwerami LSP.
---

anymous może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

anymous posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `ANYMOUS_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i anymous otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji anymous.
