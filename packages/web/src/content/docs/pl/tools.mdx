...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. anymous posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w anymous.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `ANYMOUS_EXPERIMENTAL_LSP_TOOL=true` (lub `ANYMOUS_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy anymous lub gdy zmienna środowiskowa `ANYMOUS_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu anymous:
