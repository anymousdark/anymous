...
Dzięki anymous możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie anymous używa naszego własnego motywu `anymous`.
...
anymous ma kilka wbudowanych motywów.
...
- Chcą, aby anymous pasował do wyglądu ich terminala
...
anymous obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/anymous/themes/*.json` lub `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.anymous/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.anymous/themes/*.json`
