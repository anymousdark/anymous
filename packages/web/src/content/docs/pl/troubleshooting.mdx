...
Aby debugować problemy z anymous, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\anymous\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `anymous --log-level DEBUG`.

---

## Przechowywanie danych

anymous przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/anymous/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\anymous`
...
anymous Desktop uruchamia w tle lokalny serwer anymous (sidecar `anymous-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `anymous` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/anymous/anymous.jsonc` (lub `~/.config/anymous/anymous.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/anymous/anymous.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\anymous\anymous.jsonc`
...
anymous może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/anymous/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\anymous\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby anymous mógł ją odbudować.

1. Całkowicie zamknij anymous Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/anymous`
- **Linux**: usuń `~/.cache/anymous` (lub uruchom `rm -rf ~/.cache/anymous`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\anymous`

3. Uruchom ponownie anymous Desktop.

---

### Napraw problemy z połączeniem z serwerem

anymous Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `anymous.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `ANYMOUS_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `ANYMOUS_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows anymous Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami anymous.

---

### Brak powiadomień

anymous Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla anymous w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie anymous Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji anymous Desktop):

- `anymous.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `anymous.global.dat` i `anymous.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `anymous upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `anymous models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\anymous`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. anymous dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\anymous`

2. Uruchom ponownie kod anymous, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to anymous do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
anymous wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
