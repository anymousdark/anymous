---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć anymous.
---

Wtyczki umożliwiające wyłączenie anymous poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania anymous.
...
- `.anymous/plugins/` - Wtyczki na poziomie projektu
- `~/.config/anymous/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/anymous/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/anymous/anymous.json`)
2. Konfiguracja projektu (`anymous.json`)
3. Globalny katalog wtyczek (`~/.config/anymous/plugins/`)
4. Katalog wtyczek projektu (`.anymous/plugins/`)
...
anymous uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z anymous do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do anymous.
...
:::note
Jeśli korzystasz z aplikacji komputerowej anymous, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj anymous czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do anymous:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod anymous. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://anymous-cli.vercel.app/docs/sdk).
