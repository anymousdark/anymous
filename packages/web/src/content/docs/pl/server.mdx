---
title: Serwer
description: Interakcja z serwerem anymous za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `anymous serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient anymous.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `anymous.local` |
...
Ustaw `ANYMOUS_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `anymous` lub ustaw `ANYMOUS_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `anymous serve`, jak i `anymous web`.
...
Kiedy uruchomisz `anymous`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera anymous do programowej interakcji z anymous.
:::

Ta architektura umożliwia obsługę wielu klientów przez anymous i programową interakcję z kodem otwartym.

Możesz uruchomić `anymous serve`, aby uruchomić samodzielny serwer. Jeśli masz
anymous TUI działa, `anymous serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki anymous [IDE](/docs/ide).
...
Serwer anymous udostępnia następujące interfejsy API.
