---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w anymous.
---

import config from "../../../../config.mjs"
export const console = config.console

anymous używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji anymous.
...
w `~/.local/share/anymous/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim anymous
config.
...
anymous Zen to lista modeli dostarczonych przez zespół anymous, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z anymous. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od anymous Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `anymous Zen` i przejdź do [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen).
...
Działa jak każdy inny dostawca w anymous i jest całkowicie opcjonalny w użyciu.

---

## anymous Go

anymous Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół anymous, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z anymous.

1. Uruchom polecenie `/connect` w TUI, wybierz `anymous Go` i przejdź do [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen).
...
Aby używać Amazon Bedrock z anymous:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania anymous:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `anymous.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w anymous nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować anymous do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod anymous działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji anymous.
...
anymous używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować anymous
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`anymous.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [anymous-gitlab-auth](https://www.npmjs.com/package/anymous-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem anymous:
...
Aby używać Google Vertex AI z anymous:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania anymous.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą anymous, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [anymous-helicone-session](https://github.com/H2Shami/anymous-helicone-session), aby automatycznie rejestrować każdą konwersację anymous jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja anymous będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować anymous tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować anymous tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować anymous tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem anymous. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/anymous).
:::
...
Aby korzystać z Ollama Cloud z anymous:
...
3. Skopiuj klucz API do użycia w anymous.
...
6. **Ważne**: Przed użyciem modeli chmurowych w anymous musisz pobrać informacje o modelu lokalnie:
...
anymous Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół anymous. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>anymous Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **anymous Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API anymous.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji anymous.
...
5. Możesz je także dostosować za pomocą konfiguracji anymous. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z anymous:
...
Możesz także dostosować modele za pomocą konfiguracji anymous. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z anymous. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `anymous.json` w katalogu projektu:
...
Pola `limit` pozwalają anymous zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `anymous auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację anymous i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji anymous.
