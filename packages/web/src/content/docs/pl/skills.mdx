...
Umiejętności agenta pozwalają anymous odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
anymous przeszukuje te lokalizacje:

- Project config: `.anymous/skills/<name>/SKILL.md`
- Global config: `~/.config/anymous/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu anymous przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.anymous/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/anymous/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.anymous/skills/git-release/SKILL.md` w ten sposób:
...
anymous wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `anymous.json`:
...
**Dla agentów wbudowanych** (w `anymous.json`):
