...
Możesz dodać dodatkowe informacje do anymous za pomocą _Model Context Protocol_, w skrócie MCP. anymous obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [anymous Config](https://anymous-cli.vercel.app/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/anymous`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
anymous automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, anymous:
...
Jeśli serwer wymaga uwierzytelnienia, anymous poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `anymous mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji anymous bezpieczne przechowa tokeny w `~/.local/share/anymous/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w anymous, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie anymous, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć anymous z kontem Sentry.
