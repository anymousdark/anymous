---
title: Server
description: Interager med anymous-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `anymous serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en anymous-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `anymous.local` |
...
Indstil `ANYMOUS_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `anymous`, eller indstil `ANYMOUS_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `anymous serve` og `anymous web`.
...
Når du kører `anymous` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug anymous-serveren til at interagere med anymous programmatisk.
:::

Denne arkitektur lader anymous understøtte flere klienter og lader dig interagere med anymous programmatisk.

Du kan køre `anymous serve` for at starte en selvstændig server. Hvis du har
anymous TUI kørende, vil `anymous serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af anymous [IDE](/docs/ide) plugins.
...
anymous-serveren viser følgende API'er.
