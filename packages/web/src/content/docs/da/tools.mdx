...
Her er alle de innebygde verktøyene som er tilgængelige i anymous.
...
:::note
Dette verktøyet er bare tilgængelig når `ANYMOUS_EXPERIMENTAL_LSP_TOOL=true` (eller `ANYMOUS_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger anymous-leverandøren eller når miljøvariabelen `ANYMOUS_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
