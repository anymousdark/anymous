---
title: Konfiguration
description: Ved at bruge anymous JSON konfig.
---

Du kan konfigurere anymous ved hjælp af en JSON-konfigurationsfil.

---

## Format

anymous understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/anymous`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/anymous/anymous.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`ANYMOUS_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`anymous.json` i projekt) - projektspecifikke indstillinger
5. **`.anymous` mapper** - agent, kommandoer, plugins
6. **Inline config** (`ANYMOUS_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.anymous` og `~/.config/anymous` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/anymous`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale anymous-konfiguration i `~/.config/anymous/anymous.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `anymous.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når anymous starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `ANYMOUS_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `ANYMOUS_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.anymous` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.anymous` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`anymous-cli.vercel.app/config.json`**](https://anymous-cli.vercel.app/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `anymous serve` og `anymous web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din anymous-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `anymous.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din anymous-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger anymous at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din anymous-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/anymous/agents/` eller `.anymous/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil anymous falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`anymous run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/anymous/commands/` eller `.anymous/commands/`. [Learn more here](/docs/commands).
...
anymous vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard anymous **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide anymous med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.anymous/plugins/` eller `~/.config/anymous/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse anymous til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
