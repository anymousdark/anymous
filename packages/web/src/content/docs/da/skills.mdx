...
Agentferdigheter lar anymous oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
anymous søker etter disse stedene:

- Prosjektkonfigurasjon: `.anymous/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/anymous/skills/<name>/SKILL.md`
...
For prosjektlokale stier går anymous opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.anymous/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/anymous/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.anymous/skills/git-release/SKILL.md` slik:
...
anymous viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `anymous.json`:
...
**For innebygde agenter** (i `anymous.json`):
