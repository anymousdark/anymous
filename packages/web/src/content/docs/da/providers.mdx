---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i anymous.
---

import config from "../../../../config.mjs"
export const console = config.console

anymous bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i anymous-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/anymous/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i anymous-konfigurationen.
...
anymous Zen er en liste over modeller leveret af anymous-teamet, som er blevet testet og verificeret til at fungere godt med anymous. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med anymous Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg anymous og gå til [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/auth).
...
Det fungerer som alle andre udbydere i anymous og er helt valgfrit at bruge.

---

## anymous Go

anymous Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af anymous-teamet, som er testet og verificeret til at fungere godt med anymous.

1. Kør kommandoen `/connect` i TUI, vælg `anymous Go`, og gå til [anymous-cli.vercel.app/auth](https://anymous-cli.vercel.app/zen).
...
Sådan bruger du Amazon Bedrock med anymous:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører anymous:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `anymous.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i anymous understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere anymous til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at anymous skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem anymous-konfigurationen.
...
gemme tokenet i anymous auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
anymous bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse anymous
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`anymous.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [anymous-gitlab-auth](https://www.npmjs.com/package/anymous-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `anymous.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med anymous:
...
Sådan bruger du Google Vertex AI med anymous:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem anymous, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [anymous-helicone-session](https://github.com/H2Shami/anymous-helicone-session) plugin for automatisk at logge hver anymous-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver anymous-samtale opført som en separat session.
...
Du kan konfigurere anymous til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere anymous til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere anymous til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for anymous. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/anymous) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med anymous:
...
3. Kopier API-nøglen til brug i anymous.
...
6. **Vigtigt**: Før du bruger skymodeller i anymous, skal du hente modelinformationen lokalt:
...
anymous Zen er en liste over testede og verificerede modeller leveret af anymous-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>anymous Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **anymous Zen**.

   ```txt
   /connect
   ```

3. Indtast anymous API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem anymous-konfigurationen.
...
5. Du kan også tilpasse dem gennem anymous-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med anymous:
...
Du kan også tilpasse modeller gennem anymous-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med anymous. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `anymous.json`-filen i projektkataloget:
...
`limit`-felterne lader anymous forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `anymous auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek anymous-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i anymous-konfigurationen.
