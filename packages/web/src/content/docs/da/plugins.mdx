---
title: Plugins
description: Skriv dine egne plugins for at udvide anymous.
---

Plugins giver dig mulighed for at udvide anymous ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre anymouss standardadfærd.
...
- `.anymous/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/anymous/node_modules/`.
...
1. Global konfiguration (`~/.config/anymous/anymous.json`)
2. Projektkonfiguration (`anymous.json`)
3. Global plugin-mappe (`~/.config/anymous/plugins/`)
4. Projekt plugin bibliotek (`.anymous/plugins/`)
...
anymous kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En anymous SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide anymous.
...
:::note
Hvis du bruger anymous desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå anymous i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til anymous:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som anymous kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for anymous sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://anymous-cli.vercel.app/docs/sdk) for detaljer.
