...
Med anymous kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger anymous vårt eget `anymous`-tema.
...
anymous støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/anymous/themes/*.json` eller `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.anymous/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.anymous/themes/*.json`
