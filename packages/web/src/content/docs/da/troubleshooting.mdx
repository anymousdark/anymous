...
For at fejlfinde problemer med anymous, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\anymous\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `anymous --log-level DEBUG`.

---

## Lagring

anymous gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/anymous/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\anymous`
...
anymous Desktop kører en lokal anymous-server (`anymous-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `anymous`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/anymous/anymous.jsonc` (eller `~/.config/anymous/anymous.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/anymous/anymous.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\anymous\anymous.jsonc`
...
anymous kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/anymous/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\anymous\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så anymous kan genopbygge den.

1. Afslut anymous Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/anymous`
- **Linux**: slet `~/.cache/anymous` (eller kør `rm -rf ~/.cache/anymous`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\anymous`

3. Start anymous Desktop på ny.

---

### Løs problemer med serverforbindelse

anymous Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `anymous.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `ANYMOUS_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `ANYMOUS_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver anymous Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i anymous.

---

### Meddelelser vises ikke

anymous Desktop viser kun systemvarsler når:

- varsler er aktiveret for anymous i dine OS-indstillinger, og
...
1. Afslut anymous Desktop.
2. Find og slet disse filer (de findes i anymous Desktop-appens datamappe):

- `anymous.settings.dat` (skrivebordsstandardserver URL)
- `anymous.global.dat` og `anymous.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med anymous:
...
For at finde ud af hvilke modeller du har adgang til, kør `anymous models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\anymous`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. anymous installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\anymous`

2. Start anymous på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge anymous til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
anymous vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
