...
Wenn anymous Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\anymous\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `anymous --log-level DEBUG`.

---

## Speicher

anymous speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/anymous/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\anymous`
...
anymous Desktop startet im Hintergrund einen lokalen anymous-Server (`anymous-cli`-Sidecar).
...
- Nur macOS: `anymous`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/anymous/anymous.jsonc` (oder `~/.config/anymous/anymous.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/anymous/anymous.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\anymous\anymous.jsonc`
...
anymous kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/anymous/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\anymous\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit anymous ihn neu aufbauen kann.

1. Beende anymous Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/anymous`
- **Linux**: loesche `~/.cache/anymous` (oder fuehre aus: `rm -rf ~/.cache/anymous`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\anymous`

3. Starte anymous Desktop neu.

---

### Server-Verbindungsprobleme beheben

anymous Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `anymous.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `ANYMOUS_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `ANYMOUS_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt anymous Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit anymous-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

anymous Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer anymous in den OS-Einstellungen aktiviert sind, und
...
1. Beende anymous Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von anymous Desktop):

- `anymous.settings.dat` (desktop default server URL)
- `anymous.global.dat` und `anymous.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit anymous hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `anymous models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\anymous`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. anymous installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\anymous`

2. Starte anymous neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt anymous, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
anymous erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
