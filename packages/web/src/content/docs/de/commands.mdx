...
Erstellen Sie `.anymous/commands/test.md`:

```md title=".anymous/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die anymous-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem anymous [config](/docs/config):

```json title="anymous.jsonc" {4-12}
{
...
- Global: `~/.config/anymous/commands/`
- Pro Projekt: `.anymous/commands/`

```markdown title="~/.config/anymous/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".anymous/commands/component.md"
---
...
```md title=".anymous/commands/create-file.md"
---
...
```md title=".anymous/commands/analyze-coverage.md"
---
...
```md title=".anymous/commands/review-changes.md"
---
...
```md title=".anymous/commands/review-component.md"
---
...
anymous enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
