...
Mit anymous kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet anymous das `anymous`-Theme.
...
anymous liefert mehrere eingebaute Themes mit.
...
- anymous optisch ans Terminal anpassen wollen
...
anymous unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/anymous/themes/*.json` oder `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.anymous/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.anymous/themes/*.json`
