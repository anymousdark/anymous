---
title: Regeln
description: Lege eigene Anweisungen fuer anymous fest.
---

Du kannst anymous ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in anymous den Befehl `/init` aus.
...
So kann anymous sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht anymous sie zu erweitern.
...
anymous kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/anymous/AGENTS.md` speichern.
Diese gelten in allen anymous-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt anymous dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/anymous/AGENTS.md` existiert)
...
Beim Start sucht anymous in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/anymous/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `anymous.json` oder in `~/.config/anymous/anymous.json` angeben.
...
anymous parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit anymous.json

Empfohlen ist das `instructions`-Feld in der `anymous.json`:
...
Du kannst anymous explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `anymous.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
