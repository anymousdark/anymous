---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um anymous zu erweitern.
---

Mit Plugins können Sie anymous erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von anymous zu ändern.
...
- `.anymous/plugins/` – Plugins auf Projektebene
- `~/.config/anymous/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/anymous/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/anymous/anymous.json`)
2. Projektkonfiguration (`anymous.json`)
3. Globales Plugin-Verzeichnis (`~/.config/anymous/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.anymous/plugins/`)
...
anymous führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein anymous-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie anymous erweitern können.
...
:::note
Wenn Sie die anymous-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass anymous `.env`-Dateien liest:
...
Plugins können anymous auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das anymous aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für anymous zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://anymous-cli.vercel.app/docs/sdk).
