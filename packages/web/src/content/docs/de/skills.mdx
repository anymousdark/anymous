...
Agent Skills erlauben anymous, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
anymous sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft anymous vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.anymous/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/anymous/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.anymous/skills/git-release/SKILL.md` zum Beispiel so:
...
anymous listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `anymous.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `anymous.json`):
