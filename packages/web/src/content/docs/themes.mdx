...
With anymous you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, anymous uses our own `anymous` theme.
...
anymous comes with several built-in themes.
...
- Want anymous to match their terminal's appearance
...
anymous supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/anymous/themes/*.json` or `$XDG_CONFIG_HOME/anymous/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.anymous/themes/*.json`
4. **Current working directory** - Defined in `./.anymous/themes/*.json`
